...
import (
	...
	"github.com/gitbundle/modules/pprof"
...
func setup(logPath string, debug bool) {
	...
	if debug {
		_ = log.NewLogger(1000, "console", "console", `{"level":"trace","stacktracelevel":"NONE","stderr":true}`, setting.IsProd)
	} else {
		_ = log.NewLogger(1000, "console", "console", `{"level":"fatal","stacktracelevel":"NONE","stderr":true}`, setting.IsProd)
	}
	setting.LoadFromExisting()

	// Check if setting.RepoRootPath exists. It could be the case that it doesn't exist, this can happen when
	// `[repository]` `ROOT` is a relative path and $GITBUNDLE_WORK_DIR isn't passed to the SSH connection.
	if _, err := os.Stat(setting.RepoRootPath); err != nil {
		if os.IsNotExist(err) {
			_ = fail("Incorrect configuration, no repository directory.", "Directory `[repository].ROOT` %q was not found, please check if $GITBUNDLE_WORK_DIR is passed to the SSH connection or make `[repository].ROOT` an absolute value.", setting.RepoRootPath)
		} else {
...
func fail(userMessage, logMessage string, args ...interface{}) error {
	...
	_, _ = fmt.Fprintln(os.Stderr, "GitBundle:", userMessage)
...
func runServ(c *cli.Context) error {
	...
	if setting.SSH.Disabled {
		println("GitBundle: SSH has been disabled")
		return nil
	}
	...
	if len(cmd) == 0 {
		...
		case asymkey_model.KeyTypeDeploy:
			println("Hi there! You've successfully authenticated with the deploy key named " + key.Name + ", but GitBundle does not provide shell access.")
		case asymkey_model.KeyTypePrincipal:
			println("Hi there! You've successfully authenticated with the principal " + key.Content + ", but GitBundle does not provide shell access.")
		default:
			println("Hi there, " + user.Name + "! You've successfully authenticated with the key named " + key.Name + ", but GitBundle does not provide shell access.")
		}
		println("If this is unexpected, please log in with password and setup GitBundle under another user.")
		return nil
