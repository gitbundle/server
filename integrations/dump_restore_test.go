...
func (c *compareDump) assertEquals(repoBefore, repoAfter *repo_model.Repository) {
	...
	// base.Repository
	//
	_ = c.assertEqual("repo.yml", schemas.Repository{}, compareFields{
		"Name": {
	...
	// base.Label
	//
	labels, ok := c.assertEqual("label.yml", []schemas.Label{}, compareFields{}).([]*schemas.Label)
	assert.True(c.t, ok)
	...
	// base.Milestone
	//
	milestones, ok := c.assertEqual("milestone.yml", []schemas.Milestone{}, compareFields{
	...
	//
	issues, ok := c.assertEqual("issue.yml", []schemas.Issue{}, compareFields{
	...
	for _, issue := range issues {
		...
		comments, ok := c.assertEqual(filename, []schemas.Comment{}, compareFields{
			"Index": {ignore: true},
		}).([]*schemas.Comment)
		assert.True(c.t, ok)
	...
	prs, ok := c.assertEqual("pull_request.yml", []schemas.PullRequest{}, compareFields{
		"Assignees": {ignore: true}, // not implemented yet
	...
	for _, pr := range prs {
		...
		comments, ok := c.assertEqual(filename, []schemas.Comment{}, compareFields{}).([]*schemas.Comment)
		assert.True(c.t, ok)
		for _, comment := range comments {
