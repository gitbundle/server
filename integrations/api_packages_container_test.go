...
import (
	...
	"github.com/gitbundle/modules/packages/container/oci"
...
func TestPackageContainer(t *testing.T) {
	...
	blobContent, _ := base64.StdEncoding.DecodeString(
		`H4sIAAAJbogA/2IYBaNgFIxYAAgAAP//Lq+17wAEAAA=`,
	)
		...
		t.Run(fmt.Sprintf("[Image:%s]", image), func(t *testing.T) {
			...
			t.Run("UploadBlob/Monolithic", func(t *testing.T) {
				...
				req = NewRequestWithBody(
					t,
					"POST",
					fmt.Sprintf("%s/blobs/uploads?digest=%s", url, unknownDigest),
					bytes.NewReader(blobContent),
				)
				addTokenAuthHeader(req, userToken)
				...
				req = NewRequestWithBody(
					t,
					"POST",
					fmt.Sprintf("%s/blobs/uploads?digest=%s", url, blobDigest),
					bytes.NewReader(blobContent),
				)
				addTokenAuthHeader(req, userToken)
				...
				assert.Equal(
					t,
					fmt.Sprintf("/v2/%s/%s/blobs/%s", user.Name, image, blobDigest),
					resp.Header().Get("Location"),
				)
				assert.Equal(t, blobDigest, resp.Header().Get("Docker-Content-Digest"))

				pv, err := packages_model.GetInternalVersionByNameAndVersion(
					db.DefaultContext,
					user.ID,
					packages_model.TypeContainer,
					image,
					container_model.UploadVersion,
				)
			...
			t.Run("UploadBlob/Chunked", func(t *testing.T) {
				...
				req = NewRequestWithBody(
					t,
					"PATCH",
					setting.AppURL+uploadURL[1:]+"000",
					bytes.NewReader(blobContent),
				)
				addTokenAuthHeader(req, userToken)
				...
				req = NewRequest(
					t,
					"PUT",
					fmt.Sprintf("%s?digest=%s", setting.AppURL+uploadURL[1:], blobDigest),
				)
				addTokenAuthHeader(req, userToken)
				resp = MakeRequest(t, req, http.StatusCreated)
				...
				t.Run(fmt.Sprintf("[Tag:%s]", tag), func(t *testing.T) {
					t.Run("UploadManifest", func(t *testing.T) {
						...
						req := NewRequestWithBody(
							t,
							"POST",
							fmt.Sprintf("%s/blobs/uploads?digest=%s", url, configDigest),
						...
						req = NewRequestWithBody(
							t,
							"PUT",
							fmt.Sprintf("%s/manifests/%s", url, tag),
						...
						assert.ElementsMatch(
							t,
							[]string{strings.ToLower(user.LowerName + "/" + image)},
							getAllByName(pd.PackageProperties, container_module.PropertyRepository),
						)
						assert.True(
							t,
							has(pd.VersionProperties, container_module.PropertyManifestTagged),
						)
						...
						for _, pfd := range pd.Files {
							...
							case container_model.ManifestFilename:
								...
								assert.Equal(
									...
									pfd.Properties.GetByName(container_module.PropertyMediaType),
								)
								assert.Equal(
									t,
									manifestDigest,
									pfd.Properties.GetByName(container_module.PropertyDigest),
								)
					...
					t.Run("HeadManifest", func(t *testing.T) {
						...
						assert.Equal(
							t,
							fmt.Sprintf("%d", len(manifestContent)),
			...
			t.Run("UploadUntaggedManifest", func(t *testing.T) {
				...
				req := NewRequestWithBody(
					t,
					"PUT",
					fmt.Sprintf("%s/manifests/%s", url, untaggedManifestDigest),
					strings.NewReader(untaggedManifestContent),
				)
				addTokenAuthHeader(req, userToken)
				...
				assert.Equal(
					t,
					fmt.Sprintf("%d", len(untaggedManifestContent)),
					resp.Header().Get("Content-Length"),
				)
				assert.Equal(t, untaggedManifestDigest, resp.Header().Get("Docker-Content-Digest"))
			...
			t.Run("UploadIndexManifest", func(t *testing.T) {
				...
				req := NewRequestWithBody(
					t,
					"PUT",
					fmt.Sprintf("%s/manifests/%s", url, multiTag),
					strings.NewReader(indexManifestContent),
				)
				addTokenAuthHeader(req, userToken)
				...
				assert.ElementsMatch(
					t,
					[]string{manifestDigest, untaggedManifestDigest},
					getAllByName(pd.VersionProperties, container_module.PropertyManifestReference),
				)
				...
				assert.Equal(
					...
					pd.Files[0].Properties.GetByName(container_module.PropertyMediaType),
				)
				assert.Equal(
					t,
					indexManifestDigest,
					pd.Files[0].Properties.GetByName(container_module.PropertyDigest),
				)
			})
			...
			t.Run("UploadBlob/Mount", func(t *testing.T) {
				...
				req := NewRequest(
					t,
					"POST",
					fmt.Sprintf("%s/blobs/uploads?mount=%s", url, unknownDigest),
				)
				addTokenAuthHeader(req, userToken)
				...
				req = NewRequest(
					t,
					"POST",
					fmt.Sprintf("%s/blobs/uploads?mount=%s", url, blobDigest),
				)
				addTokenAuthHeader(req, userToken)
				resp := MakeRequest(t, req, http.StatusCreated)
				...
				cases := []struct {
				...
				}{
					{
						...
						ExpectedLink: fmt.Sprintf(
							`</v2/%s/%s/tags/list?last=multi>; rel="next"`,
							user.Name,
							image,
						),
					},
					...
					},
					{
						...
						ExpectedLink: fmt.Sprintf(
							`</v2/%s/%s/tags/list?last=main&n=2>; rel="next"`,
							user.Name,
							image,
						),
					},
					...
					},
					{
						...
						ExpectedLink: fmt.Sprintf(
							`</v2/%s/%s/tags/list?last=main&n=1>; rel="next"`,
							user.Name,
							image,
						),
					},
