...
func TestMain(m *testing.M) {
	...
	if os.Getenv("GITBUNDLE_SLOW_TEST_TIME") != "" {
		duration, err := time.ParseDuration(os.Getenv("GITBUNDLE_SLOW_TEST_TIME"))
		if err == nil {
			slowTest = duration
		}
	}

	if os.Getenv("GITBUNDLE_SLOW_FLUSH_TIME") != "" {
		duration, err := time.ParseDuration(os.Getenv("GITBUNDLE_SLOW_FLUSH_TIME"))
		if err == nil {
			slowFlush = duration
		}
	}
...
func initIntegrationTest() {
	...
	if giteaConf == "" {
		fmt.Println("Environment variable $GITBUNDLE_CONF not set")
		os.Exit(1)
