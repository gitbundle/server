...
func getDashboardContextUser(ctx *context.Context) *user_model.User {
	...
	orgs, err := repoman.GetUserOrgsList(ctx.Doer)
	if err != nil {
...
func Dashboard(ctx *context.Context) {
	...
	if setting.Service.EnableUserHeatmap {
		data, err := user_heatmap_model.GetUserHeatmapDataByUserTeam(
			ctxUser,
			ctx.Org.Team,
			ctx.Doer,
		)
...
func Milestones(ctx *context.Context) {
	...
	BuildMilestonesOverview(ctx)
	ctx.HTML(http.StatusOK, tplMilestones)
}

func BuildMilestonesOverview(ctx *context.Context) {
	ctxUser := getDashboardContextUser(ctx)
	if ctx.Written() {
		return
	}
	...
	var (
		userRepoCond = repo_model.SearchRepositoryCondition(
			&repoOpts,
		) // all repo condition user could visit
	...
	milestones, err := issues_model.SearchMilestones(
		repoCond,
		page,
		isShowClosed,
		sortType,
	...
	for i := 0; i < len(milestones); {
		...
		if milestones[i].Repo == nil {
			log.Warn(
				"Cannot find milestone %d 's repository %d",
				milestones[i].ID,
				milestones[i].RepoID,
			)
...
func loadRepoByIDs(
	ctxUser *user_model.User,
	issueCountByRepo map[int64]int64,
	unitType unit.Type,
) (map[int64]*repo_model.Repository, error) {
...
func ShowGPGKeys(ctx *context.Context) {
	...
	if len(failedEntitiesID) > 0 { // If some key need re-import to be exported
		headers["Note"] = fmt.Sprintf(
			"The keys with the following IDs couldn't be exported and need to be reuploaded %s",
			strings.Join(failedEntitiesID, ", "),
		)
	...
	for _, e := range entities {
		err = e.Serialize(
			writer,
		) // TODO find why key are exported with a different cipherTypeByte as original (should not be blocking but strange)
		if err != nil {
			ctx.ServerError("ShowGPGKeys", err)
			return
		}
