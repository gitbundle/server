...
import (
	...
	"github.com/gitbundle/server/pkg/git/lfs"
	"github.com/gitbundle/server/pkg/mailer"
	"github.com/gitbundle/server/pkg/metrics"
	"github.com/gitbundle/server/pkg/perm"
	"github.com/gitbundle/server/pkg/routers/web/admin"
	"github.com/gitbundle/server/pkg/routers/web/auth"
	"github.com/gitbundle/server/pkg/routers/web/dev"
	"github.com/gitbundle/server/pkg/routers/web/events"
	...
	_ "github.com/gitbundle/server/pkg/session" // to registers all internal adapters
...
func buildAuthGroup() *authm.Group {
	group := authm.NewGroup(
		&authm.OAuth2{}, // FIXME: this should be removed and only applied in download and oauth realted routers
		&authm.Basic{},  // FIXME: this should be removed and only applied in download and git/lfs routers
		&authm.Session{},
	)
	if setting.Service.EnableReverseProxyAuth {
		group.Add(&authm.ReverseProxy{})
	}
	specialAdd(group)
...
func Routes() *web.Route {
	...
	routes.Use(
		web.WrapWithPrefix(public.AssetsURLPathPrefix, public.AssetsHandlerFunc(&public.Options{
	...
	routes.Route(
		"/avatars/*",
		"GET, HEAD",
		storageHandler(setting.Avatar.Storage, "avatars", storage.Avatars),
	)
	routes.Route(
		"/repo-avatars/*",
		"GET, HEAD",
		storageHandler(setting.RepoAvatar.Storage, "repo-avatars", storage.RepoAvatars),
	)
	...
	routes.Get("/apple-touch-icon.png", func(w http.ResponseWriter, req *http.Request) {
		http.Redirect(
			w,
			req,
			path.Join(setting.StaticURLPrefix, "/assets/img/apple-touch-icon.png"),
	...
	// NOTE: replace gzip with chi.middleware.Compress globally
	...
	if setting.Service.EnableCaptcha {
		...
		routes.Route(
			"/captcha/*",
			"GET,HEAD",
			append(common, captcha.Captchaer(context.GetImageCaptcha()))...)
	}
	...
	common = append(common, templates.Contexter())
...
func RegisterRoutes(m *web.Route) {
	...
	ignSignIn := context.Toggle(
		&context.ToggleOptions{SignInRequired: setting.Service.RequireSignInView},
	)
	ignExploreSignIn := context.Toggle(
	...
	linkAccountEnabled := func(ctx *context.Context) {
		if !setting.Service.EnableOpenIDSignIn && !setting.Service.EnableOpenIDSignUp &&
			!setting.OAuth2.Enable {
	...
	reqPackageEnabled := func(ctx *context.Context) {
	...
	m.Group("/explore", func() {
		...
		m.Get("/issues", explore.Issues)
		m.Get("/pulls", explore.Pulls)
		m.Get("/milestones", explore.Milestones)
	...
	m.Group("/user/settings", func() {
		m.Combo("").
			Get(user_setting.Profile).
			Post(bindIgnErr(forms.UpdateProfileForm{}), user_setting.ProfilePost)

		m.Combo("/change_password").
			Get(auth.MustChangePassword).
			Post(bindIgnErr(forms.MustChangePasswordForm{}), auth.MustChangePasswordPost)
		m.Group("/avatar", func() {
			m.Get("/", user_setting.Avatar)
			m.Post("/", bindIgnErr(forms.AvatarForm{}), user_setting.AvatarPost)
			m.Post("/delete", user_setting.DeleteAvatar)
		})
		m.Group("/account", func() {
			m.Combo("/password").
				Get(user_setting.Password).
				Post(bindIgnErr(forms.ChangePasswordForm{}), user_setting.PasswordPost)

			m.Group("/emails", func() {
				m.Get("/", user_setting.Emails)
				m.Post("/", bindIgnErr(forms.AddEmailForm{}), user_setting.EmailPost)
				m.Post("/delete", user_setting.DeleteEmail)
			})
	...
	m.Group("/admin", func() {
		...
		m.Group("/users", func() {
			...
			m.Combo("/new").
				Get(admin.NewUser).
				Post(bindIgnErr(forms.AdminCreateUserForm{}), admin.NewUserPost)
			m.Combo("/{userid}").
				Get(admin.EditUser).
				Post(bindIgnErr(forms.AdminEditUserForm{}), admin.EditUserPost)
		...
		m.Group("/hooks", func() {
			...
			m.Post("/gitbundle/{id}", bindIgnErr(forms.NewWebhookForm{}), repo.WebHooksEditPost)
		...
		m.Group("/{configType:default-hooks|system-hooks}", func() {
			...
			m.Post("/gitbundle/new", bindIgnErr(forms.NewWebhookForm{}), repo.GitBundleHooksNewPost)
		...
		m.Group("/auths", func() {
			...
			m.Combo("/new").
				Get(admin.NewAuthSource).
				Post(bindIgnErr(forms.AuthenticationForm{}), admin.NewAuthSourcePost)
	...
	// reqRepoOwner := context.RequireRepoOwner()
	...
	reqRepoIssuesOrPullsReader := context.RequireRepoReaderOr(
		unit.TypeIssues,
		unit.TypePullRequests,
	)
	...
	m.Get(
		"/{username}/{reponame}/releases/download/{vTag}/{fileName}",
		ignSignIn,
		context.RepoAssignment,
		repo.MustBeNotEmpty,
		repo.RedirectDownload,
	)
	...
	m.Group("/{username}/{reponame}/settings", func() {
		...
		}, repo.SettingsCtxData, func(ctx *context.Context) {
			ctx.Data["PageIsSettingsOptions"] = true
		})

		m.Group("/mirrors", func() {
			m.Get("/", repo.Mirrors)
			m.Post("/", bindIgnErr(forms.RepoSettingForm{}), repo.MirrorsPost)
		}, repo.SettingsCtxData, func(ctx *context.Context) {
			ctx.Data["PageIsSettingsMirrors"] = true
		})

		m.Group("/signing", func() {
			m.Get("/", repo.Signing)
			m.Post("/", bindIgnErr(forms.RepoSettingForm{}), repo.SigningPost)
		}, repo.SettingsCtxData, func(ctx *context.Context) {
			ctx.Data["PageIsSettingsSigning"] = true
		})

		m.Group("/admin", func() {
			m.Get("/", repo.Admin)
			m.Post("/", bindIgnErr(forms.RepoSettingForm{}), repo.AdminPost)
		}, repo.SettingsCtxData, func(ctx *context.Context) {
			ctx.Data["PageIsSettingsAdmin"] = true
		})
		...
		m.Post(
			"/rename_branch",
			bindIgnErr(forms.RenameBranchForm{}),
			context.RepoMustNotBeArchived(),
			repo.RenameBranchPost,
		)
		...
		m.Group("/tags", func() {
			...
			m.Post(
				"",
				bindIgnErr(forms.ProtectTagForm{}),
				context.RepoMustNotBeArchived(),
				repo.NewProtectedTagPost,
			)
	...
	}, reqSignIn, context.RepoAssignment, context.UnitTypes(), reqRepoAdmin, context.RepoRef(), func(ctx *context.Context) {
	...
	m.Group("/{username}/{reponame}", func() {
		...
		m.Group("/{type:issues|pulls}", func() {
			m.Group("/{index}", func() {
				...
				m.Combo("/comments").
					Post(repo.MustAllowUserComment, bindIgnErr(forms.CreateCommentForm{}), repo.NewComment)
				m.Group("/times", func() {
	...
	m.Group("/{username}/{reponame}", func() {
		...
		m.Post("/tags/delete", repo.DeleteTag, reqSignIn, repo.MustBeNotEmpty, context.RepoMustNotBeArchived(), reqRepoCodeWriter, context.RepoRef())
		m.Group("/releases", func() {
			...
			var err error
			ctx.Repo.Commit, err = ctx.Repo.GitRepo.GetBranchCommit(
				ctx.Repo.Repository.DefaultBranch,
			)
			if err != nil {
	...
	m.Group("/{username}/{reponame}", func() {
		...
		// if setting.Packages.Enabled {
		m.Get("/packages", reqPackageEnabled, repo.Packages)
		// }
		...
		m.Group("/wiki", func() {
			m.Combo("/").
				Get(repo.Wiki).
				Post(context.RepoMustNotBeArchived(), reqSignIn, reqRepoWikiWriter, bindIgnErr(forms.NewWikiForm{}), repo.WikiPost)
			m.Combo("/*").
				Get(repo.Wiki).
				Post(context.RepoMustNotBeArchived(), reqSignIn, reqRepoWikiWriter, bindIgnErr(forms.NewWikiForm{}), repo.WikiPost)
	...
	// TODO: repo render page
	m.Group("/{username}/{reponame}", func() {
		m.Get("", ignSignIn, context.RepoAssignment, context.RepoRef(), context.UnitTypes(), repo.SetEditorconfigIfExists, repo.Home)
		...
		}, ignSignInAndCsrf, context.UserAssignmentWeb())
