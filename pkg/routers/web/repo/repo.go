...
func Create(ctx *context.Context) {
	...
	if templateID > 0 {
		...
		if err == nil && repoman.CheckRepoUnitUser(ctx, templateRepo, ctxUser, unit.TypeCode) {
...
func CreatePost(ctx *context.Context) {
	...
	if form.RepoTemplate > 0 {
		...
		repo, err = repo_service.CreateRepository(ctx.Doer, ctxUser, repoman.CreateRepoOptions{
			Name:          form.RepoName,
			Description:   form.Description,
...
func RedirectDownload(ctx *context.Context) {
	...
	releases, err := release_model.GetReleasesByRepoIDAndNames(ctx, curRepo.ID, tagNames)
	if err != nil {
...
func SearchRepo(ctx *context.Context) {
	...
	for i, repo := range repos {
		results[i] = &api.Repository{
			...
			AvatarURL:     repo.RelAvatarLink(),
			Description:   repo.Description,
			LanguageStats: make([]*api.LanguageStat, 0, 1),
		}
		languageStats, err := repo_model.GetTopLanguageStats(repo, 1)
		if err != nil {
			log.Error("repo_model.GetTopLanguageStats, err: %v", err)
			continue
		}
		if len(languageStats) > 0 {
			results[i].LanguageStats = append(results[i].LanguageStats, &api.LanguageStat{
				IsPrimary:  languageStats[0].IsPrimary,
				Language:   languageStats[0].Language,
				Percentage: languageStats[0].Percentage,
				Size:       languageStats[0].Size,
				Color:      languageStats[0].Color,
			})
		}
	}
