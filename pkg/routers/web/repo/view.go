	...
	for _, entry := range entries {
		...
		for i, ext := range exts {
			if markup.IsReadmeFile(entry.Name(), ext) {
				if readmeFiles[i] == nil ||
					base.NaturalSortLess(readmeFiles[i].name, entry.Blob().Name()) {
					name := entry.Name()
		...
		if markup.IsReadmeFile(entry.Name()) {
			if readmeFiles[3] == nil ||
				base.NaturalSortLess(readmeFiles[3].name, entry.Blob().Name()) {
				name := entry.Name()
				isSymlink := entry.IsLink()
				if isSymlink {
...
func renderDirectory(ctx *context.Context, treeLink string) {
	...
	if ctx.Repo.TreePath != "" {
		ctx.Data["Title"] = ctx.Tr(
			"repo.file.title",
			ctx.Repo.Repository.Name+"/"+path.Base(ctx.Repo.TreePath),
			ctx.Repo.RefName,
		)
	}
...
func findReadmeFile(
	...
	for _, entry := range entries {
		if entry.IsDir() {
			...
			case ".gitbundle":
				if entry.Name() == ".gitbundle" || docsEntries[1] == nil {
					docsEntries[1] = entry
				}
			case ".github":
	...
	if ctx.Repo.TreePath == "" && readmeFile == nil {
		for _, entry := range docsEntries {
			...
			if readmeFile != nil {
				...
				readmeTreelink = treeLink + "/" + util.PathEscapeSegments(
	...
	if isTextFile && setting.LFS.StartServer {
		...
		if pointer.IsValid() {
			...
			if meta != nil {
				...
				ctx.Data["RawFileLink"] = fmt.Sprintf(
					"%s.git/info/lfs/objects/%s/%s",
					ctx.Repo.Repository.HTMLURL(),
					url.PathEscape(meta.Oid),
					url.PathEscape(filenameBase64),
				)
			}
		}
	}
	...
	if markupType := markup.Type(readmeFile.name); markupType != "" {
		...
		err := markup.Render(&markup.RenderContext{
			...
			RelativePath: path.Join(
				ctx.Repo.TreePath,
				readmeFile.name,
			), // ctx.Repo.TreePath is the directory not the Readme so we must append the Readme filename (and path).
...
func renderFile(ctx *context.Context, entry *git.TreeEntry, treeLink, rawLink string) {
	...
	if isTextFile && setting.LFS.StartServer {
		...
		if pointer.IsValid() {
			...
			if meta != nil {
				...
				fileSize = meta.Size
				ctx.Data["RawFileLink"] = ctx.Repo.RepoLink + "/media/" + ctx.Repo.BranchNameSubURL() + "/" + util.PathEscapeSegments(
					ctx.Repo.TreePath,
				)
			}
		}
	...
	case isRepresentableAsText:
		...
		if markupType != "" && !shouldRenderSource {
			...
			ctx.Data["EscapeStatus"], ctx.Data["FileContent"] = charset.EscapeControlString(
				result.String(),
			)
...
func checkHomeCodeViewable(ctx *context.Context) {
	if len(ctx.Repo.Units) > 0 {
		if ctx.Repo.Repository.IsBeingCreated() {
			task, err := task_model.GetMigratingTask(ctx.Repo.Repository.ID)
			if err != nil {
		...
		if ctx.IsSigned {
			...
			if err := repoman.SetRepoReadBy(ctx, ctx.Repo.Repository.ID, ctx.Doer.ID); err != nil {
...
func renderDirectoryFiles(ctx *context.Context, timeout time.Duration) git.Entries {
	...
	ctx.Data["LastCommitLoaderURL"] = ctx.Repo.RepoLink + "/lastcommit/" + url.PathEscape(
		ctx.Repo.CommitID,
	...
	if setting.CacheService.LastCommit.Enabled &&
		ctx.Repo.CommitsCount >= setting.CacheService.LastCommit.CommitsCount {
		c = git.NewLastCommitCache(
			ctx.Repo.Repository.FullName(),
			ctx.Repo.GitRepo,
			setting.LastCommitCacheTTLSeconds,
			cache.GetCache(),
		)
	}
	...
	ctx.Data["Files"], latestCommit, err = entries.GetCommitsInfo(
		commitInfoCtx,
		ctx.Repo.Commit,
		ctx.Repo.TreePath,
		c,
	...
	if latestCommit != nil {
		...
		statuses, _, err := git_model.GetLatestCommitStatus(
			ctx,
			ctx.Repo.Repository.ID,
			latestCommit.ID.String(),
			db.ListOptions{},
		)
		if err != nil {
...
func RenderUserCards(
	ctx *context.Context,
	total int,
	getter func(opts db.ListOptions) ([]*user_model.User, error),
	tpl base.TplName,
) {
...
func Watchers(ctx *context.Context) {
	...
	RenderUserCards(
		ctx,
		ctx.Repo.Repository.NumWatches,
