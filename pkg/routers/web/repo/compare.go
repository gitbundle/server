...
import (
	...
	upload_service "github.com/gitbundle/server/pkg/upload/manager"
...
func ParseCompareInfo(ctx *context.Context) *CompareInfo {
	...
	if rootRepo != nil &&
		...
		canRead := repoman.CheckRepoUnitUser(ctx, rootRepo, ctx.Doer, unit.TypeCode)
		if canRead {
	...
	if ownForkRepo != nil &&
		...
		canRead := repoman.CheckRepoUnitUser(ctx, ownForkRepo, ctx.Doer, unit.TypeCode)
		if canRead {
			ctx.Data["OwnForkRepo"] = ownForkRepo
			if !fileOnly {
...
func CompareDiff(ctx *context.Context) {
	...
	upload_service.AddUploadContext(ctx, "comment")
