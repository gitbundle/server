...
func calReleaseNumCommitsBehind(repoCtx *context.Repository, release *release_model.Release, countCache map[string]int64) error {
...
func releasesOrTags(ctx *context.Context, isTagList bool) {
	...
	opts := release_model.FindReleasesOptions{
		ListOptions:   listOptions,
		IncludeDrafts: writeAccess && !isTagList,
		IncludeTags:   isTagList,
	}

	releases, err := release_model.GetReleasesByRepoID(ctx.Repo.Repository.ID, opts)
	if err != nil {
		ctx.ServerError("GetReleasesByRepoID", err)
		return
	}

	count, err := release_model.GetReleaseCountByRepoID(ctx.Repo.Repository.ID, opts)
	if err != nil {
		ctx.ServerError("GetReleaseCountByRepoID", err)
		return
	}

	if err = release_model.GetReleaseAttachments(ctx, releases...); err != nil {
...
func SingleRelease(ctx *context.Context) {
	...
	release, err := release_model.GetRelease(ctx.Repo.Repository.ID, ctx.Params("*"))
	if err != nil {
	...
	err = release_model.GetReleaseAttachments(ctx, release)
	if err != nil {
		ctx.ServerError("GetReleaseAttachments", err)
		return
	}
	...
	ctx.Data["Releases"] = []*release_model.Release{release}
	ctx.HTML(http.StatusOK, tplReleases)
}

// LatestRelease redirects to the latest release
func LatestRelease(ctx *context.Context) {
	release, err := release_model.GetLatestReleaseByRepoID(ctx.Repo.Repository.ID)
	if err != nil {
		if release_model.IsErrReleaseNotExist(err) {
...
func NewRelease(ctx *context.Context) {
	...
	if tagName := ctx.FormString("tag"); len(tagName) > 0 {
		rel, err := release_model.GetRelease(ctx.Repo.Repository.ID, tagName)
		if err != nil && !release_model.IsErrReleaseNotExist(err) {
...
func NewReleasePost(ctx *context.Context) {
	...
	rel, err := release_model.GetRelease(ctx.Repo.Repository.ID, form.TagName)
	if err != nil {
		if !release_model.IsErrReleaseNotExist(err) {
			ctx.ServerError("GetRelease", err)
			return
		...
		if len(form.TagOnly) > 0 {
			if err = releaseservice.CreateNewTag(ctx, ctx.Doer, ctx.Repo.Repository, form.Target, form.TagName, msg); err != nil {
				...
				if release_model.IsErrInvalidTagName(err) {
		...
		if err = releaseservice.CreateRelease(ctx.Repo.GitRepo, rel, attachmentUUIDs, msg); err != nil {
			...
			switch {
			case release_model.IsErrReleaseAlreadyExist(err):
				ctx.RenderWithErr(ctx.Tr("repo.release.tag_name_already_exist"), tplReleaseNew, &form)
			case release_model.IsErrInvalidTagName(err):
				ctx.RenderWithErr(ctx.Tr("repo.release.tag_name_invalid"), tplReleaseNew, &form)
			case release_model.IsErrProtectedTagName(err):
