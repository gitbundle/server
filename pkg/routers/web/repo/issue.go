...
import (
	...
	vars "github.com/gitbundle/modules/template_vars"
	...
	comment_service "github.com/gitbundle/server/pkg/issues/comments"
	...
	pull_model "github.com/gitbundle/server/pkg/pull"
...
func MustAllowPulls(ctx *context.Context) {
	...
	if ctx.IsSigned && repo_model.HasForkedRepo(ctx.Doer.ID, ctx.Repo.Repository.ID) {
		...
		ctx.Repo.PullRequest.HeadInfoSubURL = url.PathEscape(
			ctx.Doer.Name,
		) + ":" + util.PathEscapeSegments(
			ctx.Repo.BranchName,
		)
	}
}
...
func issues(ctx *context.Context, milestoneID, projectID int64, isPullOption util.OptionalBool) {
	...
	if len(ctx.FormString("state")) == 0 && issueStats.OpenCount == 0 &&
		issueStats.ClosedCount != 0 {
		isShowClosed = true
	}
	...
	if repo.Owner.IsOrganization() {
		orgLabels, err := issues_model.GetLabelsByOrgID(
			ctx,
			repo.Owner.ID,
			ctx.FormString("sort"),
			db.ListOptions{},
	...
	ctx.Data["IssueRefEndNames"], ctx.Data["IssueRefURLs"] = issue_service.GetRefEndNamesAndURLs(
		issues,
		ctx.Repo.RepoLink,
	)
...
func Issues(ctx *context.Context) {
	...
	issues(
		ctx,
		ctx.FormInt64("milestone"),
		ctx.FormInt64("project"),
		util.OptionalBoolOf(isPullList),
	)
	if ctx.Written() {
		return
	}
...
func RetrieveRepoMilestonesAndAssignees(ctx *context.Context, repo *repo_model.Repository) {
	var err error
	ctx.Data["OpenMilestones"], _, err = issues_model.GetMilestones(
		issues_model.GetMilestonesOption{
			RepoID: repo.ID,
			State:  api.StateOpen,
		},
	)
...
func setTemplateIfExists(
	ctx *context.Context,
	ctxDataKey string,
	possibleDirs, possibleFiles []string,
) {
	templateCandidates := make([]string, 0, len(possibleFiles))
	if ctx.FormString("template") != "" {
		for _, dirName := range possibleDirs {
			templateCandidates = append(
				templateCandidates,
				path.Join(dirName, ctx.FormString("template")),
			)
		}
	}
	templateCandidates = append(
		templateCandidates,
		possibleFiles...) // Append files to the end because they should be fallback
	for _, filename := range templateCandidates {
		...
		if found {
			...
			if err != nil {
				log.Debug(
					"could not extract metadata from %s [%s]: %v",
					filename,
...
func NewIssue(ctx *context.Context) {
	...
	if milestoneID > 0 {
		milestone, err := issues_model.GetMilestoneByRepoID(
			ctx,
...
func NewIssueChooseTemplate(ctx *context.Context) {
	...
	if len(issueTemplates) == 0 {
		...
		ctx.Redirect(
			fmt.Sprintf("%s/issues/new?%s", ctx.Repo.Repository.HTMLURL(), ctx.Req.URL.RawQuery),
			http.StatusSeeOther,
		)
		return
	}
	...
	if len(form.AssigneeIDs) > 0 {
		...
		for _, aID := range assigneeIDs {
			...
			if !valid {
				ctx.ServerError(
					"canBeAssigned",
					repo_model.ErrUserDoesNotHaveAccessToRepo{UserID: aID, RepoName: repo.Name},
				)
				return nil, nil, 0, 0
			}
		}
...
func roleDescriptor(
	...
	issue *issues_model.Issue,
) (issues_model.RoleDescriptor, error) {
...
func ViewIssue(ctx *context.Context) {
	if ctx.Params(":type") == "issues" {
		...
		if err == nil && extIssueUnit != nil {
			if extIssueUnit.ExternalTrackerConfig().ExternalTrackerStyle == markup.IssueNameStyleNumeric ||
				extIssueUnit.ExternalTrackerConfig().ExternalTrackerStyle == "" {
				metas := ctx.Repo.Repository.ComposeMetas()
				metas["index"] = ctx.Params(":index")
				res, err := vars.Expand(
					extIssueUnit.ExternalTrackerConfig().ExternalTrackerFormat,
					metas,
				)
				if err != nil {
					log.Error(
						"unable to expand template vars for issue url. issue: %s, err: %v",
						metas["index"],
						err,
					)
	...
	if ctx.IsSigned {
		// Update issue-user.
		if err = repoman.SetIssueReadBy(ctx, issue.ID, ctx.Doer.ID); err != nil {
	...
	ctx.Data["CanCreateIssueDependencies"] = ctx.Repo.CanCreateIssueDependencies(
		ctx.Doer,
		issue.IsPull,
	)
	...
	if issue.IsPull {
		...
		if ctx.IsSigned {
			...
			ctx.Data["AllowMerge"], err = pull_service.IsUserAllowedToMerge(
				ctx,
				pull,
				perm,
				ctx.Doer,
		...
		defaultMergeMessage, err := pull_service.GetDefaultMergeMessage(
			ctx.Repo.GitRepo,
			pull,
			mergeStyle,
		)
		...
		defaultSquashMergeMessage, err := pull_service.GetDefaultMergeMessage(
			ctx.Repo.GitRepo,
			pull,
		...
		if pull.ProtectedBranch != nil {
			...
			ctx.Data["IsBlockedByApprovals"] = !issues_model.HasEnoughApprovals(
				ctx,
				pull.ProtectedBranch,
				pull,
			)
			ctx.Data["IsBlockedByRejection"] = issues_model.MergeBlockedByRejectedReview(
				ctx,
				pull.ProtectedBranch,
				pull,
			)
			ctx.Data["IsBlockedByOfficialReviewRequests"] = issues_model.MergeBlockedByOfficialReviewRequests(
				ctx,
				pull.ProtectedBranch,
				pull,
			)
			ctx.Data["IsBlockedByOutdatedBranch"] = issues_model.MergeBlockedByOutdatedBranch(
				pull.ProtectedBranch,
				pull,
			)
			ctx.Data["GrantedApprovals"] = issues_model.GetGrantedApprovalsCount(
				ctx,
				pull.ProtectedBranch,
				pull,
			)
		...
		if ctx.Doer != nil {
			sign, key, _, err := asymkey_service.SignMerge(
				ctx,
				pull,
				ctx.Doer,
				pull.BaseRepo.RepoPath(),
				pull.BaseBranch,
				pull.GetGitRefName(),
			)
		...
		if isPullBranchDeletable && pull.HasMerged {
			exist, err := issues_model.HasUnmergedPullRequestsByHeadInfo(
				ctx,
				pull.HeadRepoID,
				pull.HeadBranch,
			)
		...
		ctx.Data["HasPendingPullRequestMerge"], ctx.Data["PendingPullRequestMerge"], err = pull_model.GetScheduledMergeByPullID(
			ctx,
			pull.ID,
		)
		if err != nil {
	...
	ctx.Data["SignInLink"] = setting.AppSubURL + "/user/login?redirect_to=" + url.QueryEscape(
		ctx.Data["Link"].(string),
	)
	...
	if ctx.IsSigned {
		...
		hiddenCommentTypes, _ = new(
			big.Int,
		).SetString(val, 10)
		// we can safely ignore the failed conversion here
	}
...
func getActionIssues(ctx *context.Context) []*issues_model.Issue {
	...
	for _, issue := range issues {
		if issue.RepoID != ctx.Repo.Repository.ID {
			ctx.NotFound(
				"some issue's RepoID is incorrect",
				errors.New("some issue's RepoID is incorrect"),
			)
			return nil
		}
...
func GetIssueInfo(ctx *context.Context) {
	issue, err := issues_model.GetIssueWithAttrsByIndex(
		ctx.Repo.Repository.ID,
		ctx.ParamsInt64(":index"),
	)
	if err != nil {
		if issues_model.IsErrIssueNotExist(err) {
...
func UpdateIssueTitle(ctx *context.Context) {
	...
	if !ctx.IsSigned ||
		(!issue.IsPoster(ctx.Doer.ID) && !ctx.Repo.CanWriteIssuesOrPulls(issue.IsPull)) {
...
func UpdateIssueContent(ctx *context.Context) {
	...
	if !ctx.IsSigned ||
		(ctx.Doer.ID != issue.PosterID && !ctx.Repo.CanWriteIssuesOrPulls(issue.IsPull)) {
		ctx.Error(http.StatusForbidden)
		return
	}
...
func UpdatePullReviewRequest(ctx *context.Context) {
	...
	for _, issue := range issues {
		...
		if reviewID < 0 {
			...
			err = issue_service.IsValidTeamReviewRequest(
				ctx,
				team,
				ctx.Doer,
				action == "attach",
		...
		err = issue_service.IsValidReviewRequest(
			ctx,
			reviewer,
			ctx.Doer,
			action == "attach",
	...
	if ctx.FormString("team") != "" {
		if ctx.FormString("owner") == "" {
			ctx.Error(
				...
				"",
				"Owner organisation is required for filtering on team",
			)
			return
		}
...
func ListIssues(ctx *context.Context) {
	...
	var labelIDs []int64
	if len(keyword) > 0 {
		issueIDs, err = issue_indexer.SearchIssuesByKeyword(
			ctx,
			[]int64{ctx.Repo.Repository.ID},
			keyword,
		)
		if err != nil {
			ctx.Error(http.StatusInternalServerError, err.Error())
...
func NewComment(ctx *context.Context) {
	...
	if !ctx.IsSigned ||
		(ctx.Doer.ID != issue.PosterID && !ctx.Repo.CanReadIssuesOrPulls(issue.IsPull)) {
		if log.IsTrace() {
			if ctx.IsSigned {
				issueType := "issues"
				if issue.IsPull {
				...
				log.Trace(
					"Permission Denied: User %-v not the Poster (ID: %d) and cannot read %s in Repo %-v.\n"+
	...
	defer func() {
		...
		if (ctx.Repo.CanWriteIssuesOrPulls(issue.IsPull) || (ctx.IsSigned && issue.IsPoster(ctx.Doer.ID))) &&
			...
			if form.Status == "reopen" && issue.IsPull {
				pull := issue.PullRequest
				var err error
				pr, err = issues_model.GetUnmergedPullRequest(
					pull.HeadRepoID,
					pull.BaseRepoID,
					pull.HeadBranch,
					pull.BaseBranch,
	...
	comment, err := comment_service.CreateIssueComment(
		ctx.Doer,
		ctx.Repo.Repository,
		issue,
		form.Content,
...
func UpdateCommentContent(ctx *context.Context) {
	...
	if !ctx.IsSigned ||
		(ctx.Doer.ID != comment.PosterID && !ctx.Repo.CanWriteIssuesOrPulls(comment.Issue.IsPull)) {
		ctx.Error(http.StatusForbidden)
		return
	}
	...
	if comment.Type != issues_model.CommentTypeComment &&
		comment.Type != issues_model.CommentTypeReview &&
		comment.Type != issues_model.CommentTypeCode {
		ctx.Error(http.StatusNoContent)
		return
	}
...
// TODO: issue with comments for reaction has bug
...
func ChangeIssueReaction(ctx *context.Context) {
	...
	case "react":
		...
		log.Trace(
			"Reaction for issue created: %d/%d/%d",
			ctx.Repo.Repository.ID,
			issue.ID,
			reaction.ID,
		)
...
func ChangeCommentReaction(ctx *context.Context) {
	...
	if !ctx.IsSigned ||
		(ctx.Doer.ID != comment.PosterID && !ctx.Repo.CanReadIssuesOrPulls(comment.Issue.IsPull)) {
		if log.IsTrace() {
			if ctx.IsSigned {
				issueType := "issues"
	...
	switch ctx.Params(":action") {
	case "react":
		reaction, err := issues_model.CreateCommentReaction(
			ctx.Doer.ID,
			comment.Issue.ID,
			comment.ID,
			form.Content,
		...
		log.Trace(
			"Reaction for comment created: %d/%d/%d/%d",
			ctx.Repo.Repository.ID,
			comment.Issue.ID,
			comment.ID,
			reaction.ID,
	...
	case "unreact":
		...
		log.Trace(
			"Reaction for comment removed: %d/%d/%d",
			ctx.Repo.Repository.ID,
			comment.Issue.ID,
			comment.ID,
		)
...
func filterXRefComments(ctx *context.Context, issue *issues_model.Issue) error {
	...
	for i := 0; i < len(issue.Comments); {
		c := issue.Comments[i]
		if issues_model.CommentTypeIsRef(c.Type) && c.RefRepoID != issue.RepoID &&
			c.RefRepoID != 0 {
			var err error
				...
				if cur.Content != "1" {
					...
					for i, label := range prev.AddedLabels {
						if cur.Label.ID == label.ID {
							prev.AddedLabels = append(
								prev.AddedLabels[:i],
								prev.AddedLabels[i+1:]...)
