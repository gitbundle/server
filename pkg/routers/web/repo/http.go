...
func httpBase(ctx *context.Context) (h *serviceHandler) {
	if setting.Repository.DisableHTTPGit {
		...
		_, err := ctx.Resp.Write(
			[]byte("Interacting with repositories by HTTP protocol is not allowed"),
		)
		if err != nil {
			log.Error(err.Error())
		}
	...
	if len(setting.Repository.AccessControlAllowOrigin) > 0 {
		...
		ctx.Resp.Header().
			Set("Access-Control-Allow-Headers", "Content-Type, Authorization, User-Agent")
	...
	if !owner.IsOrganization() && !owner.IsActive {
		ctx.PlainText(
			...
			"Repository cannot be accessed. You cannot push or open issues/pull-requests.",
		)
		return
	}
		...
		if ctx.IsBasicAuth && ctx.Data["IsApiToken"] != true {
			...
			if err == nil {
				...
				ctx.PlainText(
					...
					"Users with two-factor authentication enabled cannot perform HTTP/HTTPS operations via plain username and password. Please create and use a personal access token on the user settings page",
				)
				return
...
func dummyInfoRefs(ctx *context.Context) {
	infoRefsOnce.Do(func() {
		...
		refs, _, err := git.NewCommand(ctx, "receive-pack", "--stateless-rpc", "--advertise-refs", ".").
			RunStdBytes(&git.RunOpts{Dir: tmpDir})
		if err != nil {
...
var safeGitProtocolHeader = regexp.MustCompile(
...
func serviceRPC(ctx gocontext.Context, h serviceHandler, service string) {
	...
	if protocol := h.r.Header.Get("Git-Protocol"); protocol != "" &&
		safeGitProtocolHeader.MatchString(protocol) {
...
func GetInfoRefs(ctx *context.Context) {
	...
	if hasAccess(ctx, getServiceType(h.r), *h, false) {
		...
		refs, _, err := git.NewCommand(ctx, service, "--stateless-rpc", "--advertise-refs", ".").
			RunStdBytes(&git.RunOpts{Env: h.environ, Dir: h.dir})
		if err != nil {
			log.Error(fmt.Sprintf("%v - %s", err, string(refs)))
		}
...
func GetPackFile(ctx *context.Context) {
	...
	if h != nil {
		...
		h.sendFile(
			...
			"objects/pack/pack-"+ctx.Params("file")+".pack",
		)
	}
}
...
func GetIdxFile(ctx *context.Context) {
	...
	if h != nil {
		...
		h.sendFile(
			...
			"objects/pack/pack-"+ctx.Params("file")+".idx",
		)
	}
}
