...
import (
	...
	upload_manager "github.com/gitbundle/server/pkg/upload/manager"
...
func editFilePost(ctx *context.Context, form forms.EditRepoFileForm, isNewFile bool) {
	...
	if _, err := files_service.CreateOrUpdateRepoFile(ctx, ctx.Repo.Repository, ctx.Doer, &files_service.UpdateRepoFileOptions{
		...
		if git.IsErrNotExist(err) {
		...
		} else if repo_model.IsErrFilenameInvalid(err) {
			ctx.Data["Err_TreePath"] = true
			ctx.RenderWithErr(ctx.Tr("repo.editor.filename_is_invalid", form.TreePath), tplEditFile, &form)
		} else if repo_model.IsErrFilePathInvalid(err) {
			ctx.Data["Err_TreePath"] = true
			if fileErr, ok := err.(repo_model.ErrFilePathInvalid); ok {
				switch fileErr.Type {
		...
		} else if repo_model.IsErrRepoFileAlreadyExists(err) {
			ctx.Data["Err_TreePath"] = true
		...
		} else if repo_model.IsErrBranchAlreadyExists(err) {
			// For when a user specifies a new branch that already exists
			ctx.Data["Err_NewBranchName"] = true
			if branchErr, ok := err.(repo_model.ErrBranchAlreadyExists); ok {
			...
			if len(errPushRej.Message) == 0 {
				...
				flashError, err := ctx.RenderToString(tplAlertDetails, map[string]interface{}{
					...
					"Details": setting.SanitizeFlashErrorString(errPushRej.Message),
				})
				if err != nil {
			...
			flashError, err := ctx.RenderToString(tplAlertDetails, map[string]interface{}{
				...
				"Details": setting.SanitizeFlashErrorString(err.Error()),
			})
			if err != nil {
				ctx.ServerError("editFilePost.HTMLString", err)
				return
...
func DeleteFilePost(ctx *context.Context) {
	...
	if _, err := files_service.DeleteRepoFile(ctx, ctx.Repo.Repository, ctx.Doer, &files_service.DeleteRepoFileOptions{
		...
		if git.IsErrNotExist(err) || repo_model.IsErrRepoFileDoesNotExist(err) {
		...
		} else if repo_model.IsErrCommitIDDoesNotMatch(err) || git.IsErrPushOutOfDate(err) {
	...
	if form.CommitChoice == frmCommitChoiceDirect {
...
func UploadFile(ctx *context.Context) {
	...
	upload_manager.AddUploadContext(ctx, "repo")
	canCommit := renderCommitRights(ctx)
	treePath := cleanUploadFileName(ctx.Repo.TreePath)
	if treePath != ctx.Repo.TreePath {
...
func UploadFilePost(ctx *context.Context) {
	...
	if err := files_service.UploadRepoFiles(ctx, ctx.Repo.Repository, ctx.Doer, &files_service.UploadRepoFileOptions{
		...
		if git_model.IsErrLFSFileLocked(err) {
			...
			ctx.Data["Err_TreePath"] = true
			fileErr := err.(repo_model.ErrFilePathInvalid)
			switch fileErr.Type {
			case git.EntryModeSymlink:
...
func UploadFileToServer(ctx *context.Context) {
	...
	err = upload_manager.Verify(buf, header.Filename, setting.Repository.Upload.AllowedTypes)
	if err != nil {
	...
	upload, err := upload.NewUpload(name, buf, file)
	if err != nil {
...
func RemoveUploadFileFromServer(ctx *context.Context) {
	...
	if err := upload.DeleteUploadByUUID(form.File); err != nil {
