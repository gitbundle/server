...
var pullRequestTemplateCandidates = []string{
	...
	"pull_request_template.md",
	".gitbundle/PULL_REQUEST_TEMPLATE.md",
	".gitbundle/pull_request_template.md",
...
func getForkRepository(ctx *context.Context) *repo_model.Repository {
	...
	ctx.Data["IsPrivate"] = forkRepo.IsPrivate ||
		forkRepo.Owner.Visibility == structs.VisibleTypePrivate
	canForkToUser := forkRepo.OwnerID != ctx.Doer.ID &&
		!repo_model.HasForkedRepo(ctx.Doer.ID, forkRepo.ID)
...
func ForkPost(ctx *context.Context) {
	...
	if err != nil {
		...
		case db.IsErrNameReserved(err):
			ctx.RenderWithErr(
				ctx.Tr("repo.form.name_reserved", err.(db.ErrNameReserved).Name),
				tplFork,
				&form,
			)
		case db.IsErrNamePatternNotAllowed(err):
			ctx.RenderWithErr(
				ctx.Tr(
					"repo.form.name_pattern_not_allowed",
					err.(db.ErrNamePatternNotAllowed).Pattern,
				),
				tplFork,
				&form,
			)
...
func PrepareMergedViewPullInfo(ctx *context.Context, issue *issues_model.Issue) *git.CompareInfo {
	...
	if pull.MergeBase == "" {
		...
		if commitSHA != "" {
			...
			parentCommit, _, err = git.NewCommand(ctx, "rev-list", "-1", "--skip=1", commitSHA).
				RunStdString(&git.RunOpts{Dir: ctx.Repo.GitRepo.Path})
			if err == nil {
	...
	if len(compareInfo.Commits) != 0 {
		...
		commitStatuses, _, err := git_model.GetLatestCommitStatus(
			ctx,
...
func PrepareViewPullInfo(ctx *context.Context, issue *issues_model.Issue) *git.CompareInfo {
	...
	ctx.Data["EnableStatusCheck"] = pull.ProtectedBranch != nil &&
		pull.ProtectedBranch.EnableStatusCheck
	...
	if headBranchExist {
		var err error
		ctx.Data["UpdateAllowed"], ctx.Data["UpdateByRebaseAllowed"], err = pull_service.IsUserAllowedToUpdate(
			ctx,
			pull,
			ctx.Doer,
		)
	...
	if pull.ProtectedBranch != nil && pull.ProtectedBranch.EnableStatusCheck {
		...
		ctx.Data["RequiredStatusCheckState"] = pull_service.MergeRequiredContextsCommitStatus(
			commitStatuses,
			pull.ProtectedBranch.StatusCheckContexts,
		)
	}
...
func UpdatePullRequest(ctx *context.Context) {
	...
	allowedUpdateByMerge, allowedUpdateByRebase, err := pull_service.IsUserAllowedToUpdate(
		ctx,
		issue.PullRequest,
		ctx.Doer,
	)
	...
	message := fmt.Sprintf(
		...
		issue.PullRequest.BaseBranch,
		issue.PullRequest.HeadBranch,
	)

	if err = pull_service.Update(ctx, issue.PullRequest, ctx.Doer, message, rebase); err != nil {
		if repo_model.IsErrMergeConflicts(err) {
			conflictError := err.(repo_model.ErrMergeConflicts)
			flashError, err := ctx.RenderToString(tplAlertDetails, map[string]interface{}{
				"Message": ctx.Tr("repo.pulls.merge_conflict"),
				"Summary": ctx.Tr("repo.pulls.merge_conflict_summary"),
				"Details": setting.SanitizeFlashErrorString(
					conflictError.StdErr,
				) + "<br>" + setting.SanitizeFlashErrorString(
			...
			ctx.Redirect(issue.Link())
			return
		} else if repo_model.IsErrRebaseConflicts(err) {
			conflictError := err.(repo_model.ErrRebaseConflicts)
			flashError, err := ctx.RenderToString(tplAlertDetails, map[string]interface{}{
				"Message": ctx.Tr("repo.pulls.rebase_conflict", setting.SanitizeFlashErrorString(conflictError.CommitSHA)),
				"Summary": ctx.Tr("repo.pulls.rebase_conflict_summary"),
				"Details": setting.SanitizeFlashErrorString(conflictError.StdErr) + "<br>" + setting.SanitizeFlashErrorString(conflictError.StdOut),
			})
			if err != nil {
				ctx.ServerError("UpdatePullRequest.HTMLString", err)
				return
...
func MergePullRequest(ctx *context.Context) {
	...
	if err := pull_service.CheckPullMergable(ctx, ctx.Doer, &ctx.Repo.Permission, pr, manuallMerge, forceMerge); err != nil {
		if errors.Is(err, pull_service.ErrIsClosed) {
			...
			ctx.Redirect(issue.Link())
		} else if repo_model.IsErrDisallowedToMerge(err) {
			ctx.Flash.Error(ctx.Tr("repo.pulls.no_merge_not_ready"))
			ctx.Redirect(issue.Link())
	...
	if manuallMerge {
		if err := pull_service.MergedManually(pr, ctx.Doer, ctx.Repo.GitRepo, form.MergeCommitID); err != nil {
			if repo_model.IsErrInvalidMergeStyle(err) {
	...
	if len(message) == 0 {
		var err error
		message, err = pull_service.GetDefaultMergeMessage(
			ctx.Repo.GitRepo,
			pr,
	...
	if form.MergeWhenChecksSucceed {
		...
		// schedule auto merge
		scheduled, err := automerge.ScheduleAutoMerge(
			ctx,
			ctx.Doer,
			pr,
			repo_model.MergeStyle(form.Do),
	...
	if err := pull_service.Merge(ctx, pr, ctx.Doer, ctx.Repo.GitRepo, repo_model.MergeStyle(form.Do), form.HeadCommitID, message); err != nil {
		if repo_model.IsErrInvalidMergeStyle(err) {
			ctx.Flash.Error(ctx.Tr("repo.pulls.invalid_merge_option"))
			ctx.Redirect(issue.Link())
		} else if repo_model.IsErrMergeConflicts(err) {
			conflictError := err.(repo_model.ErrMergeConflicts)
			flashError, err := ctx.RenderToString(tplAlertDetails, map[string]interface{}{
			...
			ctx.Flash.Error(flashError)
			ctx.Redirect(issue.Link())
		} else if repo_model.IsErrMergeUnrelatedHistories(err) {
			...
			ctx.Redirect(issue.Link())
		} else if repo_model.IsErrSHADoesNotMatch(err) {
			...
			if len(message) == 0 {
				...
				flashError, err := ctx.RenderToString(tplAlertDetails, map[string]interface{}{
					...
					"Details": setting.SanitizeFlashErrorString(pushrejErr.Message),
				})
				if err != nil {
					ctx.ServerError("MergePullRequest.HTMLString", err)
					return
	...
	if form.DeleteBranchAfterMerge {
		...
		if ctx.Repo != nil && ctx.Repo.Repository != nil &&
			pr.HeadRepoID == ctx.Repo.Repository.ID &&
...
func CleanUpPullRequest(ctx *context.Context) {
	...
	if ctx.Repo != nil && ctx.Repo.Repository != nil && ctx.Repo.Repository.ID == pr.BaseRepoID &&
		ctx.Repo.GitRepo != nil {
	...
	if pr.BaseRepoID != pr.HeadRepoID && ctx.Repo != nil && ctx.Repo.Repository != nil &&
		ctx.Repo.Repository.ID == pr.HeadRepoID &&
		ctx.Repo.GitRepo != nil {
		gitRepo = ctx.Repo.GitRepo
...
func DownloadPullDiffOrPatch(ctx *context.Context, patch bool) {
	pr, err := issues_model.GetPullRequestByIndex(
		ctx,
		ctx.Repo.Repository.ID,
...
func UpdatePullRequestTarget(ctx *context.Context) {
	...
	if err := pull_service.ChangeTargetBranch(ctx, pr, ctx.Doer, targetBranch); err != nil {
		if issues_model.IsErrPullRequestAlreadyExists(err) {
			...
			errorMessage := ctx.Tr(
				"repo.pulls.has_pull_request",
				html.EscapeString(ctx.Repo.RepoLink+"/pulls/"+strconv.FormatInt(err.IssueID, 10)),
				html.EscapeString(RepoRelPath),
				err.IssueID,
			) // FIXME: Creates url insidde locale string
		...
		} else if repo_model.IsErrPullRequestHasMerged(err) {
		...
		} else if repo_model.IsErrBranchesEqual(err) {
