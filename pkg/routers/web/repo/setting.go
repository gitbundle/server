...
import (
	...
	"github.com/gitbundle/server/pkg/indexer/code"
...
const (
	...
	tplSettingsMirrors base.TplName = "repo/settings/mirrors"
	tplSettingsSigning base.TplName = "repo/settings/security/signing"
	tplSettingsAdmin   base.TplName = "repo/settings/security/admin"
	tplCollaboration   base.TplName = "repo/settings/access/collaboration"
	tplBranches        base.TplName = "repo/settings/branches"
	tplTags            base.TplName = "repo/settings/tags"
	tplGithooks        base.TplName = "repo/settings/githooks"
	tplGithookEdit     base.TplName = "repo/settings/githook_edit"
	tplDeployKeys      base.TplName = "repo/settings/security/deploy_keys"
...
func Mirrors(ctx *context.Context) {
	ctx.HTML(http.StatusOK, tplSettingsMirrors)
}

func MirrorsPost(ctx *context.Context) {
	form := web.GetForm(ctx).(*forms.RepoSettingForm)
	...
	case "mirror":
		...
		ctx.Flash.Success(ctx.Tr("repo.settings.mirror_settings.update_success"))
		ctx.Redirect(repo.Link() + "/settings/mirrors")
...
func Signing(ctx *context.Context) {
	ctx.HTML(http.StatusOK, tplSettingsSigning)
}

func SigningPost(ctx *context.Context) {
	form := web.GetForm(ctx).(*forms.RepoSettingForm)
	...
	case "signing":
		...
		ctx.Flash.Success(ctx.Tr("repo.settings.signing_settings.update_success"))
		ctx.Redirect(ctx.Repo.RepoLink + "/settings/signing")
...
func Admin(ctx *context.Context) {
	ctx.HTML(http.StatusOK, tplSettingsAdmin)
}

func AdminPost(ctx *context.Context) {
	form := web.GetForm(ctx).(*forms.RepoSettingForm)
	...
	case "admin":
		...
		ctx.Flash.Success(ctx.Tr("repo.settings.admin_settings.update_success"))
		ctx.Redirect(ctx.Repo.RepoLink + "/settings/admin")
	...
	case "convert_fork":
		...
		if !ctx.Repo.Owner.CanCreateRepo() {
			...
			ctx.Flash.Error(msg)
			ctx.Redirect(repo.Link() + "/settings/admin")
			return
		}
	...
	case "transfer":
		...
		if err := repo_service.StartRepositoryTransfer(ctx.Doer, newOwner, repo, nil); err != nil {
			...
			} else if repoman.IsErrRepoTransferInProgress(err) {
	...
	case "cancel_transfer":
		...
		if err := repoman.CancelRepositoryTransfer(ctx.Repo.Repository); err != nil {
...
func CollaborationPost(ctx *context.Context) {
	name := setting.RemoveUsernameParameterSuffix(strings.ToLower(ctx.FormString("collaborator")))
	...
	if err = repoman.AddCollaborator(ctx.Repo.Repository, u); err != nil {
...
func DeleteCollaboration(ctx *context.Context) {
	if err := repoman.DeleteCollaboration(ctx.Repo.Repository, ctx.FormInt64("id")); err != nil {
...
func AddTeamPost(ctx *context.Context) {
	...
	name := setting.RemoveUsernameParameterSuffix(strings.ToLower(ctx.FormString("team")))
	if len(name) == 0 {
		ctx.Redirect(ctx.Repo.RepoLink + "/settings/collaboration")
		return
	}
	...
	if err = repoman.AddRepository(team, ctx.Repo.Repository); err != nil {
...
func DeleteTeam(ctx *context.Context) {
	...
	if err = repoman.RemoveRepository(team, ctx.Repo.Repository.ID); err != nil {
