...
func TeamsAction(ctx *context.Context) {
	...
	case "join":
		...
		err = repoman.AddTeamMember(ctx.Org.Team, ctx.Doer.ID)
	case "leave":
		err = repoman.RemoveTeamMember(ctx.Org.Team, ctx.Doer.ID)
		if err != nil {
	...
	case "remove":
		...
		err = repoman.RemoveTeamMember(ctx.Org.Team, uid)
		if err != nil {
			if organization.IsErrLastOrgOwner(err) {
				ctx.Flash.Error(ctx.Tr("form.last_org_owner"))
			} else {
	...
	case "add":
		...
		uname := setting.RemoveUsernameParameterSuffix(strings.ToLower(ctx.FormString("uname")))
		...
		if ctx.Org.Team.IsMember(u.ID) {
			...
			err = repoman.AddTeamMember(ctx.Org.Team, u.ID)
		}
...
func TeamsRepoAction(ctx *context.Context) {
	...
	case "add":
		...
		if err != nil {
			if repo_model.IsErrRepoNotExist(err) {
				...
				ctx.Redirect(
					ctx.Org.OrgLink + "/teams/" + url.PathEscape(
						ctx.Org.Team.LowerName,
					) + "/repositories",
				)
				return
			}
		...
		err = repoman.AddRepository(ctx.Org.Team, repo)
	case "remove":
		err = repoman.RemoveRepository(ctx.Org.Team, ctx.FormInt64("repoid"))
	case "addall":
		err = repoman.AddAllRepositories(ctx.Org.Team)
	case "removeall":
		err = repoman.RemoveAllRepositories(ctx.Org.Team)
	}
	...
	if action == "addall" || action == "removeall" {
		ctx.JSON(http.StatusOK, map[string]interface{}{
			"redirect": ctx.Org.OrgLink + "/teams/" + url.PathEscape(
				ctx.Org.Team.LowerName,
			) + "/repositories",
		})
		return
...
func NewTeamPost(ctx *context.Context) {
	...
	if err := repoman.NewTeam(t); err != nil {
...
func EditTeamPost(ctx *context.Context) {
	...
	if err := repoman.UpdateTeam(t, isAuthChanged, isIncludeAllChanged); err != nil {
		ctx.Data["Err_TeamName"] = true
		switch {
		case organization.IsErrTeamAlreadyExist(err):
			ctx.RenderWithErr(ctx.Tr("form.team_name_been_taken"), tplTeamNew, &form)
...
func DeleteTeam(ctx *context.Context) {
	if err := repoman.DeleteTeam(ctx.Org.Team); err != nil {
