...
import (
	...
	"github.com/gitbundle/modules/hcaptcha"
	"github.com/gitbundle/modules/log"
	"github.com/gitbundle/modules/recaptcha"
	...
	"github.com/gitbundle/server/pkg/user/externalaccount"
...
const (
	// TplActivate template path for activate user, defined in `pkg/context`, as of cycle import
...
func SignIn(ctx *context.Context) {
	...
	ctx.Data["PageIsLogin"] = !ctx.FormBool("auth_with_sspi")
	ctx.Data["PageIsSspi"] = ctx.FormBool("auth_with_sspi")
...
func SignInPost(ctx *context.Context) {
	...
	if setting.Service.EnableCaptcha {
		...
		default:
			ctx.ServerError(
				...
				fmt.Errorf("Unknown Captcha Type: %s", setting.Service.CaptchaType),
			)
			return
		}
		if err != nil {
	...
	if err != nil {
		if user_model.IsErrUserNotExist(err) || user_model.IsErrEmailAddressNotExist(err) {
			...
			log.Info(
				"Failed authentication attempt for %s from %s: %v",
				form.UserName,
				ctx.RemoteAddr(),
				err,
			)
	...
	if skipper, ok := source.Cfg.(auth_service.LocalTwoFASkipper); ok &&
		skipper.IsSkipLocalTwoFA() {
...
func handleSignIn(ctx *context.Context, u *user_model.User, remember bool) {
	...
	// handleSignInForInternalToken(ctx, u)
	// if err := registry.CreateAuthWhenNotExists(u.ID, u.Name); err != nil {
	// 	log.Error("routes/web/auth: %v", err)
	// 	ctx.ServerError("AddAuthSK", err)
	// 	return
	// }
...
//TODO: do we really need this?
// func handleSignInForInternalToken(ctx *context.Context, u *user_model.User) {
// 	t, err := access_token_model.GetInternalTokenByName("login", u.ID)
// 	if err != nil {
// 		ctx.ServerError("GetInternalTokenByName", err)
// 		return
// 	}

// 	if t.ID == 0 {
// 		if err := access_token_model.NewInternalToken("login", u.ID); err != nil {
// 			ctx.ServerError("NewInternalToken", err)
// 			return
// 		}
// 	} else {
// 		if err := t.Refresh(); err != nil {
// 			log.Error("InternalToken.Refresh : %v", err)
// 		}
// 	}
// }

func handleSignInFull(
	ctx *context.Context,
	u *user_model.User,
	remember, obeyRedirect bool,
) string {
	if remember {
	...
	if len(u.Language) == 0 {
		...
		if err := user_model.UpdateUserCols(ctx, u, "language"); err != nil {
			ctx.ServerError(
				"UpdateUserCols Language",
				fmt.Errorf("Error updating user language [user: %d, locale: %s]", u.ID, u.Language),
			)
			return setting.AppSubURL + "/"
		}
	}
	...
	if redirectTo := ctx.GetCookie("redirect_to"); len(redirectTo) > 0 &&
		!setting.IsExternalURL(redirectTo) {
		middleware.DeleteRedirectToCookie(ctx.Resp)
		if obeyRedirect {
...
func createUserInContext(
	...
	if err := user_model.CreateUser(u, overwrites); err != nil {
		...
		case db.IsErrNameReserved(err):
			ctx.Data["Err_UserName"] = true
			ctx.RenderWithErr(
				ctx.Tr("user.form.name_reserved", err.(db.ErrNameReserved).Name),
				tpl,
				form,
			)
		case db.IsErrNamePatternNotAllowed(err):
			...
			ctx.RenderWithErr(
				ctx.Tr(
					"user.form.name_pattern_not_allowed",
		...
		case db.IsErrNameCharsNotAllowed(err):
			ctx.Data["Err_UserName"] = true
			ctx.RenderWithErr(
				ctx.Tr("user.form.name_chars_not_allowed", err.(db.ErrNameCharsNotAllowed).Name),
				tpl,
				form,
			)
		default:
...
func Activate(ctx *context.Context) {
	...
	if len(code) == 0 {
		...
		if setting.Service.RegisterEmailConfirm {
			if setting.CacheService.Enabled &&
				ctx.Cache.IsExist("MailResendLimit_"+ctx.Doer.LowerName) {
...
func handleAccountActivation(ctx *context.Context, user *user_model.User) {
	...
	if err := user_model.ActivateUserEmail(user.ID, user.Email, true); err != nil {
		log.Error(
			"Unable to activate email for user: %-v with email: %s: %v",
			user,
			user.Email,
			err,
		)
	...
	if _, err := session.RegenerateSession(ctx.Resp, ctx.Req); err != nil {
		log.Error(
			"Unable to regenerate session for user: %-v with email: %s: %v",
			user,
			user.Email,
			err,
		)
