	...
	return func(next http.Handler) http.Handler {
		if storageSetting.ServeDirect {
			return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
				...
				if err != nil {
					...
					http.Error(
						w,
						fmt.Sprintf("Error whilst getting URL for %s %s", prefix, rPath),
		...
		return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
			...
			if err != nil {
				...
				http.Error(
					w,
					fmt.Sprintf("Error whilst opening %s %s", prefix, rPath),
					http.StatusInternalServerError,
				)
				return
			}
			...
			if err != nil {
				...
				http.Error(
					w,
					fmt.Sprintf("Error whilst rendering %s %s", prefix, rPath),
					http.StatusInternalServerError,
				)
				return
			}
