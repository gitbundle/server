...
func ListAccessTokens(ctx *context.APIContext) {
	...
	opts := access_token.ListAccessTokensOptions{UserID: ctx.Doer.ID, ListOptions: utils.GetListOptions(ctx)}

	count, err := access_token.CountAccessTokens(opts)
	if err != nil {
		ctx.InternalServerError(err)
		return
	}
	tokens, err := access_token.ListAccessTokens(opts)
	if err != nil {
		ctx.InternalServerError(err)
		return
	}
...
func CreateAccessToken(ctx *context.APIContext) {
	...
	exist, err := access_token.AccessTokenByNameExists(t)
	if err != nil {
		ctx.InternalServerError(err)
		return
	}
	...
	if err := access_token.NewAccessToken(t); err != nil {
...
func DeleteAccessToken(ctx *context.APIContext) {
	...
	if tokenID == 0 {
		tokens, err := access_token.ListAccessTokens(access_token.ListAccessTokensOptions{
			Name:   token,
			UserID: ctx.Doer.ID,
		})
		if err != nil {
	...
	if err := access_token.DeleteAccessTokenByID(tokenID, ctx.Doer.ID); err != nil {
		if access_token.IsErrAccessTokenNotExist(err) {
			ctx.NotFound()
		} else {
