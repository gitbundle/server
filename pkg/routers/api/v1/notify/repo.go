...
func statusStringToNotificationStatus(status string) repoman.NotificationStatus {
	switch strings.ToLower(strings.TrimSpace(status)) {
	case "unread":
		return repoman.NotificationStatusUnread
	case "read":
		return repoman.NotificationStatusRead
	case "pinned":
		return repoman.NotificationStatusPinned
	default:
		return 0
	}
}

func statusStringsToNotificationStatuses(statuses, defaultStatuses []string) []repoman.NotificationStatus {
	if len(statuses) == 0 {
		statuses = defaultStatuses
	}
	results := make([]repoman.NotificationStatus, 0, len(statuses))
...
func ListRepoNotifications(ctx *context.APIContext) {
	...
	totalCount, err := repoman.CountNotifications(opts)
	if err != nil {
		ctx.InternalServerError(err)
		return
	}

	nl, err := repoman.GetNotifications(ctx, opts)
	if err != nil {
		ctx.InternalServerError(err)
		return
	}
