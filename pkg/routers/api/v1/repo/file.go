...
func DeleteFile(ctx *context.APIContext) {
	...
	if fileResponse, err := files_service.DeleteRepoFile(ctx, ctx.Repo.Repository, ctx.Doer, opts); err != nil {
		if git.IsErrBranchNotExist(err) || repo_model.IsErrRepoFileDoesNotExist(err) || git.IsErrNotExist(err) {
			ctx.Error(http.StatusNotFound, "DeleteFile", err)
			return
		} else if repo_model.IsErrBranchAlreadyExists(err) ||
			repo_model.IsErrFilenameInvalid(err) ||
			repo_model.IsErrSHADoesNotMatch(err) ||
			repo_model.IsErrCommitIDDoesNotMatch(err) ||
			repo_model.IsErrSHAOrCommitIDNotProvided(err) {
			ctx.Error(http.StatusBadRequest, "DeleteFile", err)
			return
		} else if repo_model.IsErrUserCannotCommit(err) {
