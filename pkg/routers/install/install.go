...
import (
	...
	db_install "github.com/gitbundle/server/pkg/db/install"
...
func Install(ctx *context.Context) {
	...
	form.NsqHost = "127.0.0.1:4150"
	form.RedisHost = "redis://127.0.0.1:6379"
...
func checkDatabase(ctx *context.Context, form *forms.InstallForm) bool {
	...
	if err = db.InitEngine(ctx); err != nil {
		if strings.Contains(err.Error(), `Unknown database type: sqlite3`) {
			...
			ctx.RenderWithErr(ctx.Tr("install.sqlite3_not_available", "https://docs.gitbundle.com/installation/install-from-binary/"), tplInstall, form)
		} else {
	...
	if hasPostInstallationUser && dbMigrationVersion > 0 {
		log.Error("The database is likely to have been used by GitBundle before, database migration version=%d", dbMigrationVersion)
		...
		log.Info("User confirmed reinstallation of GitBundle into a pre-existing database")
	}

	if hasPostInstallationUser || dbMigrationVersion > 0 {
		log.Info("GitBundle will be installed in a database with: hasPostInstallationUser=%v, dbMigrationVersion=%v", hasPostInstallationUser, dbMigrationVersion)
	}
...
func SubmitInstall(ctx *context.Context) {
	...
	// Prepare AppDataPath, it is very important for GitBundle
	...
	cfg.Section("nsq").Key("CLUSTER_TCP_ADDR").SetValue(form.NsqHost)
	cfg.Section("redis").Key("CONNECTION").SetValue(form.RedisHost)
	cfg.Section("plugin-store").Key("PATH").SetValue(form.PluginInstallPath)
