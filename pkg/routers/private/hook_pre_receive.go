...
import (
	...
	perm_model "github.com/gitbundle/server/pkg/perm"
...
func preReceiveBranch(ctx *preReceiveContext, oldCommitID, newCommitID, refFullName string) {
	...
	if len(globs) > 0 {
		...
		if err != nil {
			if !repo_model.IsErrFilePathProtected(err) {
			...
			protectedFilePath = err.(repo_model.ErrFilePathProtected).Path
		}
	}
	...
	if !canPush {
		...
		if err := pull_service.CheckPullBranchProtections(ctx, pr, true); err != nil {
			if repo_model.IsErrDisallowedToMerge(err) {
