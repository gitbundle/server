...
) (*packages_model.PackageVersion, *packages_model.PackageFile, error) {
...
func createPackageAndVersion(
	...
	pvci *PackageCreationInfo,
	allowDuplicate bool,
) (*packages_model.PackageVersion, bool, error) {
	log.Trace(
		"Creating package: %v, %v, %v, %s, %s, %+v, %+v, %v",
		pvci.Creator.ID,
		pvci.Owner.ID,
		pvci.PackageType,
		pvci.Name,
		pvci.Version,
		pvci.PackageProperties,
		pvci.VersionProperties,
		allowDuplicate,
	)
...
func addFileToPackageVersion(
	ctx context.Context,
	pv *packages_model.PackageVersion,
	pfci *PackageFileCreationInfo,
) (*packages_model.PackageFile, *packages_model.PackageBlob, bool, error) {
...
func GetFileStreamByPackageNameAndVersion(
	...
	pvi *PackageInfo,
	pfi *PackageFileInfo,
) (io.ReadCloser, *packages_model.PackageFile, error) {
	log.Trace(
		"Getting package file stream: %v, %v, %s, %s, %s, %s",
		pvi.Owner.ID,
		pvi.PackageType,
		pvi.Name,
		pvi.Version,
