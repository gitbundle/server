...
import (
	...
	"github.com/gitbundle/modules/encrypt"
...
type Context struct {
	Resp     ResponseWriter
	Req      *http.Request
	ReqRaw   *http.Request          // used by reverse proxy handler
	...
	// DeploySetting    *deployment.DeploySetting
	SystemSettingMap map[string]bool
	// DeployLocked  bool
	// DeployOff     bool
}
...
// IsUserRepoWriterSpecific returns true if current user can read current repo's specific part
func (ctx *Context) IsUserRepoWriterSpecific(unitType unit.Type) bool {
	return ctx.Repo.CanWrite(unitType)
}
...
func (ctx *Context) RedirectToFirst(location ...string) {
	for _, loc := range location {
		...
		u, err := url.Parse(loc)
		if err != nil ||
			((u.Scheme != "" || u.Host != "") && !strings.HasPrefix(strings.ToLower(loc), strings.ToLower(setting.AppURL))) {
			continue
		}
...
func (ctx *Context) HTML(status int, name base.TplName) {
	...
	if err := ctx.Render.HTML(ctx.Resp, status, string(name), BaseVars().Merge(ctx.Data)); err != nil {
...
// func (ctx *Context) RedirectWithFlashErr(location, flashMsg string, logErr error, status ...int) {
// 	if logErr != nil {
// 		log.ErrorWithSkip(2, "%v", logErr)
// 	}
// 	ctx.Flash.ErrorMsg = flashMsg
// 	ctx.Data["Flash"] = ctx.Flash
...
func (ctx *Context) plainTextInternal(skip, status int, bs []byte) {
	...
	if _, err := ctx.Resp.Write(bs); err != nil {
		log.ErrorWithSkip(
			skip,
			"plainTextInternal (status=%d): write bytes failed: %v",
			status,
			err,
		)
	}
...
func (ctx *Context) JSONError(status int, err error) {
...
func (ctx *Context) RedirectWithParams(
	path string,
	params map[string]string,
	anchor ...string,
) {
	index, last := 1, len(params)
	sb := strings.Builder{}
	sb.WriteString(path)
	if last > 0 {
		sb.WriteByte('?')
	...
	if len(anchor) > 0 {
		sb.WriteByte('#')
		sb.WriteString(anchor[0])
	}
	ctx.Redirect(sb.String())
}
...
func (ctx *Context) CookieDecrypt(secret, val string) (string, bool) {
	...
	text, err = encrypt.AesGcmDecrypt(key, text)
...
func (ctx *Context) CookieEncrypt(secret, value string) string {
	key := pbkdf2.Key([]byte(secret), []byte(secret), 1000, 16, sha256.New)
	text, err := encrypt.AesGcmEncrypt(key, []byte(value))
	if err != nil {
...
func (ctx *Context) AppendAccessControlExposeHeaders(names ...string) {
	...
	if len(val) != 0 {
		ctx.RespHeader().
			Set("Access-Control-Expose-Headers", fmt.Sprintf("%s, %s", val, strings.Join(names, ", ")))
	} else {
