...
type Repository struct {
	...
	RefName      string
	BranchName   string //NOTE: means the current access branch
	TagName      string
	TreePath     string
	CommitID     string
	RepoLink     string
...
func (r *Repository) CanEnableEditor(user *user_model.User) bool {
	return r.IsViewBranch && r.CanWriteToBranch(user, r.BranchName) &&
...
func (r *Repository) CanCommitToBranch(
	ctx context.Context,
	doer *user_model.User,
) (CanCommitToBranchResults, error) {
	...
	sign, keyID, _, err := asymkey_service.SignCRUDAction(
		ctx,
		r.Repository.RepoPath(),
		doer,
		r.Repository.RepoPath(),
		git.BranchPrefix+r.BranchName,
	)
...
func (r *Repository) CanUseTimetracker(issue *issues_model.Issue, user *user_model.User) bool {
	...
	return r.Repository.IsTimetrackerEnabled() &&
		(!r.Repository.AllowOnlyContributorsToTrackTime() ||
...
func (r *Repository) GetCommitsCount() (int64, error) {
	...
	if r.IsViewBranch {
		clusterName = r.BranchName
	} else if r.IsViewTag {
		clusterName = r.TagName
	} else {
		clusterName = r.CommitID
	}
	return cache.GetInt64(
		r.Repository.GetCommitsCountCacheKey(clusterName, r.IsViewBranch || r.IsViewTag),
		func() (int64, error) {
...
func (r *Repository) GetCommitGraphsCount(
	ctx context.Context,
	hidePRRefs bool,
	branches, files []string,
) (int64, error) {
	cacheKey := fmt.Sprintf(
		"commits-count-%d-graph-%t-%s-%s",
		r.Repository.ID,
		hidePRRefs,
		branches,
		files,
...
func RepoAssignment(ctx *Context) (cancel context.CancelFunc) {
	...
	if ctx.IsSigned {
		...
		vh := &view_history.ViewHistory{
			DoerID:    ctx.Doer.ID,
			RepoID:    repo.ID,
			VisitType: view_history.RepoType,
			Content:   content,
		}
		err := vh.CreateOrUpdate(ctx)
		if err != nil {
			ctx.ServerError("CreateOrUpdateRepoViewHistory", err)
			return
		}
	}
	...
	ctx.Data["NumTags"], err = release_model.GetReleaseCountByRepoID(
		ctx.Repo.Repository.ID,
		release_model.FindReleasesOptions{
			IncludeTags: true,
		},
	)
	if err != nil {
	...
	ctx.Data["NumReleases"], err = release_model.GetReleaseCountByRepoID(
		ctx.Repo.Repository.ID,
		release_model.FindReleasesOptions{},
	)
	if err != nil {
	...
	ctx.Data["ShowForkModal"] = len(userAndOrgForks) > 1 ||
		(canSignedUserFork && len(userAndOrgForks) > 0)
	...
	if err != nil {
		if strings.Contains(err.Error(), "repository does not exist") ||
			...
			log.Error(
				"Repository %-v has a broken repository on the file system: %s Error: %v",
				ctx.Repo.Repository,
	...
	if len(ctx.Repo.BranchName) == 0 {
		if len(ctx.Repo.Repository.DefaultBranch) > 0 &&
			gitRepo.IsBranchExist(ctx.Repo.Repository.DefaultBranch) {
	...
	ctx.Data["IsPackagesEnableForAllOrgs"] = setting.Packages.EnableForAllOrgs || (!setting.Packages.EnableForAllOrgs && ctx.ContextUser.EnabledPackages)
