...
func checkDBConsistency(ctx context.Context, logger log.Logger, autofix bool) error {
	...
	if err := db.InitEngineWithMigration(ctx, migrations.EnsureUpToDate); err != nil {
		logger.Critical("Model version on the database does not match the current GitBundle version. Model consistency will not be checked until the database is upgraded")
		return err
	}
	...
	consistencyChecks := []consistencyCheck{
		...
		// find null archived repositories
		{
			...
			Counter:      action.CountNullArchivedRepository,
			Fixer:        action.FixNullArchivedRepository,
			FixedMessage: "Fixed",
		},
		...
		},
		{
			...
			Counter:      action.CountActionCreatedUnixString,
			Fixer:        action.FixActionCreatedUnixString,
