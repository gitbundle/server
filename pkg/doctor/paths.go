...
func checkConfigurationFiles(ctx context.Context, logger log.Logger, autofix bool) error {
	if fi, err := os.Stat(setting.CustomConf); err != nil || !fi.Mode().IsRegular() {
		...
		logger.Error("If you've never ran GitBundle yet, this is normal and '%s' will be created for you on first run.", setting.CustomConf)
	...
	configurationFiles := []configurationFile{
		...
		//directly add this for static assets not packed
		{"Static File Root Path", setting.StaticRootPath, true, true, false},
	}
