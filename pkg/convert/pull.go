...
func ToSimplePullRequest(
	ctx context.Context,
	pr *issues_model.PullRequest,
	doer *user_model.User,
) *api.SimplePullRequest {
	pullRequest := &api.SimplePullRequest{
	...
	if pr.Flow == issues_model.PullRequestFlowAGit {
		...
		pullRequest.Head.Sha, err = gitRepo.GetRefCommitID(pr.GetGitRefName())
		if err != nil {
			log.Error("GetRefCommitID[%s]: %v", pr.GetGitRefName(), err)
			return nil
		}
		pullRequest.Head.Name = ""
	}
	...
	if pr.HeadRepo != nil && pr.Flow == issues_model.PullRequestFlowGithub {
		...
		if git.IsErrBranchNotExist(err) {
			headCommitID, err := headGitRepo.GetRefCommitID(pullRequest.Head.Ref)
			if err != nil && !git.IsErrNotExist(err) {
				log.Error("GetCommit[%s]: %v", pr.HeadBranch, err)
				return nil
			}
			if err == nil {
				pullRequest.Head.Sha = headCommitID
			}
		} else {
			commit, err := headBranch.GetCommit()
			if err != nil && !git.IsErrNotExist(err) {
			...
			if err == nil {
				pullRequest.Head.Ref = pr.HeadBranch
				pullRequest.Head.Sha = commit.ID.String()
			}
		}
	}

	if len(pullRequest.Head.Sha) == 0 && len(pullRequest.Head.Ref) != 0 {
		baseGitRepo, err := git.OpenRepository(ctx, pr.BaseRepo.RepoPath())
		if err != nil {
			log.Error("OpenRepository[%s]: %v", pr.BaseRepo.RepoPath(), err)
			return nil
		...
		defer baseGitRepo.Close()
		refs, err := baseGitRepo.GetRefsFiltered(pullRequest.Head.Ref)
		if err != nil {
			log.Error("GetRefsFiltered[%s]: %v", pullRequest.Head.Ref, err)
			return nil
		} else if len(refs) == 0 {
			log.Error("unable to resolve PR head ref")
		} else {
			pullRequest.Head.Sha = refs[0].Object.String()
		}
	}
