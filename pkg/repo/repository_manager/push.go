...
func PushUpdateAddDeleteTags(repo *repo_model.Repository, gitRepo *git.Repository, addTags, delTags []string) error {
	return db.WithTx(func(ctx context.Context) error {
		if err := release.PushUpdateDeleteTagsContext(ctx, repo, delTags); err != nil {
			return err
		}
...
func pushUpdateAddTags(ctx context.Context, repo *repo_model.Repository, gitRepo *git.Repository, tags []string) error {
	...
	releases, err := release.GetReleasesByRepoIDAndNames(ctx, repo.ID, lowerTags)
	if err != nil {
		return fmt.Errorf("GetReleasesByRepoIDAndNames: %v", err)
	}
	relMap := make(map[string]*release.Release)
	for _, rel := range releases {
		relMap[rel.LowerTagName] = rel
	}

	newReleases := make([]*release.Release, 0, len(lowerTags)-len(relMap))
	...
	for i, lowerTag := range lowerTags {
		...
		if !has {
			...
			if err = release.UpdateRelease(ctx, rel); err != nil {
