...
import (
	...
	"github.com/gitbundle/modules/sync"
...
func TransferOwnership(doer, newOwner *user_model.User, repo *repo_model.Repository, teams []*organization.Team) error {
	...
	if err := repoman.TransferOwnership(doer, newOwner.Name, repo); err != nil {
	...
	for _, team := range teams {
		if err := repoman.AddRepository(team, newRepo); err != nil {
			return err
		}
	}
...
func StartRepositoryTransfer(doer, newOwner *user_model.User, repo *repo_model.Repository, teams []*organization.Team) error {
	if err := repoman.TestRepositoryReadyForTransfer(repo.Status); err != nil {
		return err
	}
	...
	if !hasAccess {
		if err := repoman.AddCollaborator(repo, newOwner); err != nil {
			return err
		}
	...
	if err := repoman.CreatePendingRepositoryTransfer(doer, newOwner, repo.ID, teams); err != nil {
		return err
	}
