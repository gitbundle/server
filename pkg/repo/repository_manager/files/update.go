...
func CreateOrUpdateRepoFile(ctx context.Context, repo *repo_model.Repository, doer *user_model.User, opts *UpdateRepoFileOptions) (*structs.FileResponse, error) {
	...
	if opts.NewBranch != opts.OldBranch {
		...
		if existingBranch != nil {
			return nil, repo_model.ErrBranchAlreadyExists{
	...
	if treePath == "" {
		return nil, repo_model.ErrFilenameInvalid{
			Path: opts.TreePath,
		}
	}
	...
	if hasOldBranch {
		...
		if !opts.IsNewFile {
			...
			if opts.SHA != "" {
				...
				if opts.SHA != fromEntry.ID.String() {
					return nil, repo_model.ErrSHADoesNotMatch{
						Path:       treePath,
						GivenSHA:   opts.SHA,
				...
				return nil, repo_model.ErrSHAOrCommitIDNotProvided{}
			}
		...
		for index, part := range treePathParts {
			...
			if index < len(treePathParts)-1 {
				if !entry.IsDir() {
					return nil, repo_model.ErrFilePathInvalid{
				...
				return nil, repo_model.ErrRepoFileAlreadyExists{
					Path: treePath,
				}
			}
	...
	if protectedBranch != nil {
		...
		for _, pat := range patterns {
			if pat.Match(strings.ToLower(treePath)) {
				return repo_model.ErrFilePathProtected{
					Path: treePath,
				}
			}
		}
