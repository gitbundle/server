...
import (
	...
	"github.com/gitbundle/server/pkg/repo/attachment"
...
func TestRelease_Create(t *testing.T) {
	...
	assert.NoError(t, CreateRelease(gitRepo, &release_model.Release{
		RepoID:       repo.ID,
		Repo:         repo,
		PublisherID:  user.ID,
		Publisher:    user,
...
func TestRelease_Update(t *testing.T) {
	...
	release, err := release_model.GetRelease(repo.ID, "v1.1.1")
	assert.NoError(t, err)
	releaseCreatedUnix := release.CreatedUnix
	time.Sleep(2 * time.Second) // sleep 2 seconds to ensure a different timestamp
	release.Note = "Changed note"
	assert.NoError(t, UpdateRelease(user, gitRepo, release, nil, nil, nil))
	release, err = release_model.GetReleaseByID(db.DefaultContext, release.ID)
	assert.NoError(t, err)
	...
	release, err = release_model.GetRelease(repo.ID, "v1.2.1")
	assert.NoError(t, err)
	releaseCreatedUnix = release.CreatedUnix
	time.Sleep(2 * time.Second) // sleep 2 seconds to ensure a different timestamp
	...
	release, err = release_model.GetRelease(repo.ID, "v1.3.1")
	assert.NoError(t, err)
	releaseCreatedUnix = release.CreatedUnix
	time.Sleep(2 * time.Second) // sleep 2 seconds to ensure a different timestamp
	release.Title = "Changed title"
	...
	assert.NoError(t, release_model.GetReleaseAttachments(db.DefaultContext, release))
