...
func MigrateRepositoryGitData(ctx context.Context, u *user_model.User,
	repo *repo_model.Repository, opts schemas.MigrateOptions,
...
func SyncReleasesWithTags(repo *repo_model.Repository, gitRepo *git.Repository) error {
	...
	opts := release.FindReleasesOptions{
		IncludeDrafts: true,
		IncludeTags:   true,
	...
	for page := 1; ; page++ {
		opts.Page = page
		rels, err := release.GetReleasesByRepoID(repo.ID, opts)
		if err != nil {
		...
		for _, rel := range rels {
			...
			if git.IsErrNotExist(err) || commitID != rel.Sha1 {
				if err := release.PushUpdateDeleteTag(repo, rel.TagName); err != nil {
...
func PushUpdateAddTag(repo *repo_model.Repository, gitRepo *git.Repository, tagName, sha1, refname string) error {
	...
	return release.SaveOrUpdateTag(repo, &rel)
}
...
func pullMirrorReleaseSync(repo *repo_model.Repository, gitRepo *git.Repository) error {
	...
	err = db.WithTx(func(ctx context.Context) error {
		...
		//
		if _, err := db.DeleteByBean(ctx, &release.Release{RepoID: repo.ID}); err != nil {
