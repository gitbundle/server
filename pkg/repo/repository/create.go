...
func CreateRepository(doer, u *user_model.User, opts repoman.CreateRepoOptions) (*repo_model.Repository, error) {
	...
	if err := db.WithTx(func(ctx context.Context) error {
		if err := repoman.CreateRepository(ctx, doer, u, repo, false); err != nil {
			return err
		}
		...
		if isExist {
			...
			//
			// Previously GitBundle would just delete and start afresh - this was naughty.
		...
		if rollbackRepo != nil {
			if errDelete := repoman.DeleteRepository(doer, rollbackRepo.OwnerID, rollbackRepo.ID); errDelete != nil {
...
func UpdateRepository(ctx context.Context, repo *repo_model.Repository, visibilityChanged bool) (err error) {
	...
	if visibilityChanged {
		...
		if repo.IsPrivate {
			_, err = e.Where("repo_id = ?", repo.ID).Cols("is_private").Update(&action.Action{
				IsPrivate: true,
			})
			if err != nil {
				return err
