...
# AUTO GENERATED BY GITBUNDLE, DO NOT MODIFY
data=$(cat)
exitcodes=""
hookname=$(basename $0)
...
func createDelegateHooks(repoPath string) (err error) {
	...
	for i, hookName := range hookNames {
		...
		if err = util.Remove(oldHookPath); err != nil && !os.IsNotExist(err) {
			return fmt.Errorf(
				"unable to pre-remove old hook file '%s' prior to rewriting: %v ",
				oldHookPath,
				err,
			)
		}
		...
		if err = util.Remove(newHookPath); err != nil && !os.IsNotExist(err) {
			return fmt.Errorf(
				"unable to pre-remove new hook file '%s' prior to rewriting: %v",
				newHookPath,
				err,
			)
		}
...
func CheckDelegateHooks(repoPath string) ([]string, error) {
	...
	for i, hookName := range hookNames {
		...
		if err != nil {
			results = append(
				results,
				fmt.Sprintf("unable to check if %s exists. Error: %v", oldHookPath, err),
			)
		}
		if err == nil && !isExist {
		...
		if err != nil {
			results = append(
				results,
				fmt.Sprintf("unable to check if %s exists. Error: %v", oldHookPath+".d", err),
			)
		}
		if err == nil && !isExist {
			results = append(
				results,
				fmt.Sprintf("hooks directory %s does not exist", oldHookPath+".d"),
			)
			cont = true
		}
		...
		if err != nil {
			results = append(
				results,
				fmt.Sprintf("unable to check if %s exists. Error: %v", newHookPath, err),
			)
		}
		if err == nil && !isExist {
		...
		if !checkExecutable(oldHookPath) {
			results = append(
				results,
				fmt.Sprintf("old hook file %s is not executable", oldHookPath),
			)
		}
		...
		if !checkExecutable(newHookPath) {
			results = append(
				results,
				fmt.Sprintf("new hook file %s is not executable", newHookPath),
			)
		}
	}
	return results, nil
