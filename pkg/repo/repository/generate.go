...
// GiteaTemplate holds information about a .gitbundle/template file
...
// Globs parses the .gitbundle/template globs or returns them if they were already parsed
...
func checkGiteaTemplate(tmpDir string) (*GiteaTemplate, error) {
	gtPath := filepath.Join(tmpDir, ".gitbundle", "template")
...
func generateRepoCommit(ctx context.Context, repo, templateRepo, generateRepo *repo_model.Repository, tmpDir string) error {
	...
	if gt != nil {
		if err := util.Remove(gt.Path); err != nil {
			return fmt.Errorf("remove .gitbundle/template: %v", err)
		}
...
func GenerateRepository(ctx context.Context, doer, owner *user_model.User, templateRepo *repo_model.Repository, opts GenerateRepoOptions) (_ *repo_model.Repository, err error) {
	...
	if err = repoman.CreateRepository(ctx, doer, owner, generateRepo, false); err != nil {
		return nil, err
	}
