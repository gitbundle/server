...
func TestTeam_AddMember(t *testing.T) {
	...
	test := func(teamID, userID int64) {
		...
		assert.NoError(t, repoman.AddTeamMember(team, userID))
...
func TestTeam_RemoveMember(t *testing.T) {
	...
	testSuccess := func(teamID, userID int64) {
		team := unittest.AssertExistsAndLoadBean(t, &organization.Team{ID: teamID}).(*organization.Team)
		assert.NoError(t, repoman.RemoveTeamMember(team, userID))
	...
	err := repoman.RemoveTeamMember(team, 2)
...
func TestTeam_HasRepository(t *testing.T) {
	...
	test := func(teamID, repoID int64, expected bool) {
		team := unittest.AssertExistsAndLoadBean(t, &organization.Team{ID: teamID}).(*organization.Team)
		assert.Equal(t, expected, repoman.HasRepository(team, repoID))
	}
	test(1, 1, false)
	test(1, 3, true)
	test(1, 5, true)
...
func TestTeam_AddRepository(t *testing.T) {
	...
	testSuccess := func(teamID, repoID int64) {
		...
		assert.NoError(t, repoman.AddRepository(team, repo))
	...
	assert.Error(t, repoman.AddRepository(team, repo))
...
func TestTeam_RemoveRepository(t *testing.T) {
	...
	testSuccess := func(teamID, repoID int64) {
		team := unittest.AssertExistsAndLoadBean(t, &organization.Team{ID: teamID}).(*organization.Team)
		assert.NoError(t, repoman.RemoveRepository(team, repoID))
...
func TestNewTeam(t *testing.T) {
	...
	assert.NoError(t, repoman.NewTeam(team))
...
func TestUpdateTeam(t *testing.T) {
	...
	assert.NoError(t, repoman.UpdateTeam(team, true, false))
...
func TestUpdateTeam2(t *testing.T) {
	...
	team.Name = "Owners"
	team.Description = strings.Repeat("A long description!", 100)
	err := repoman.UpdateTeam(team, true, false)
...
func TestDeleteTeam(t *testing.T) {
	...
	team := unittest.AssertExistsAndLoadBean(t, &organization.Team{ID: 2}).(*organization.Team)
	assert.NoError(t, repoman.DeleteTeam(team))
...
func TestRepository_RecalculateAccesses3(t *testing.T) {
	...
	assert.NoError(t, repoman.AddTeamMember(team5, user29.ID))
