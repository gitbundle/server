...
import (
	...
	"github.com/gitbundle/modules/avatar"
...
func DeleteUser(u *user_model.User) error {
	...
	if err != nil {
		...
		return user_model.ErrUserOwnRepos{UID: u.ID}
	}
	...
	if err != nil {
		...
		return user_model.ErrUserHasOrgs{UID: u.ID}
	}
	...
	if ownsPackages, err := packages_model.HasOwnerPackages(ctx, u.ID); err != nil {
		...
		return user_model.ErrUserOwnPackages{UID: u.ID}
	}

	if err := user_heatmap_model.DeleteUser(ctx, u); err != nil {
...
func DeleteInactiveUsers(ctx context.Context, olderThan time.Duration) error {
	...
	for _, u := range users {
		...
		if err := DeleteUser(u); err != nil {
			...
			if user_model.IsErrUserOwnRepos(err) || user_model.IsErrUserHasOrgs(err) || user_model.IsErrUserOwnPackages(err) {
				continue
			}
			return err
		}
