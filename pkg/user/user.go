...
import (
	...
	"github.com/gitbundle/server/pkg/auth/openid"
...
type User struct {
	...
	// the user is a GitBundle admin, who can access all repositories and the admin pages.
	IsAdmin bool
	// true: the user is only allowed to see organizations/repositories that they has explicit rights to.
	// (ex: in private GitBundle instances user won't be allowed to see even organizations/repositories that are set as public)
	...
	// For custom orgination plugin usage for setting.Plugin.EnableForAllOrgs is false
	// Value comes from license.PluginMetadata
	// Individual user not supportted
	Plugins []string

	// Packages feature on-off for specific organizations, or users maybe in futrue.
	// false or null means no packages feature support for the organzation or user
...
var (
	//TODO: Provide a admin ui for this reserved word?
...
func CountActiveIndividualUsers(ctx context.Context) int64 {
	count, _ := db.GetEngine(ctx).Where(builder.Eq{"is_active": true, "type": UserTypeIndividual}).Count(&User{})
	return count
}
...
func ChangeUserName(u *User, newUserName string) (err error) {
	...
	/*
		//TODO: is this the best for updating the auth_sk?
		if err = registry.UnsafeUpdateAuthUsername(ctx, u.ID, oldUserName, newUserName); err != nil {
			return err
		}
	*/
