...
`,
	...
	for _, testcase := range tests {
		t.Run(testcase.name, func(t *testing.T) {
			got, err := gitdiff.ParsePatch(setting.Git.MaxGitDiffLines, setting.Git.MaxGitDiffLineCharacters, setting.Git.MaxGitDiffFiles, strings.NewReader(testcase.gitdiff), testcase.skipTo)
...
`,
	...
	for _, testcase := range tests {
		t.Run(testcase.name, func(t *testing.T) {
			got, err := gitdiff.ParsePatch(setting.Git.MaxGitDiffLines, setting.Git.MaxGitDiffLineCharacters, setting.Git.MaxGitDiffFiles, strings.NewReader(testcase.gitdiff), "")
			if (err != nil) != testcase.wantErr {
				t.Errorf("ParsePatch(%q) error = %v, wantErr %v", testcase.name, err, testcase.wantErr)
				return
			}
...
`
	...
	result, err := gitdiff.ParsePatch(20, setting.Git.MaxGitDiffLineCharacters, setting.Git.MaxGitDiffFiles, strings.NewReader(diff), "")
	if err != nil {
	...
	result, err = gitdiff.ParsePatch(40, setting.Git.MaxGitDiffLineCharacters, setting.Git.MaxGitDiffFiles, strings.NewReader(diff), "")
	if err != nil {
		t.Errorf("There should not be an error: %v", err)
	}
	if result.Files[0].IsIncomplete {
	...
	result, err = gitdiff.ParsePatch(40, 5, setting.Git.MaxGitDiffFiles, strings.NewReader(diff), "")
	if err != nil {
		t.Errorf("There should not be an error: %v", err)
	}
	if !result.Files[0].IsIncomplete {
...
`
	...
	result, err = gitdiff.ParsePatch(20, 4096, setting.Git.MaxGitDiffFiles, strings.NewReader(diff), "")
	if err != nil {
		t.Errorf("There should not be an error: %v", err)
	}
	if !result.Files[0].IsIncomplete {
	...
	result, err = gitdiff.ParsePatch(40, 4096, setting.Git.MaxGitDiffFiles, strings.NewReader(diff), "")
	if err != nil {
		t.Errorf("There should not be an error: %v", err)
	}
	if !result.Files[0].IsIncomplete {
...
+ cut off
+ cut off`
	result, err = gitdiff.ParsePatch(setting.Git.MaxGitDiffLines, setting.Git.MaxGitDiffLineCharacters, setting.Git.MaxGitDiffFiles, strings.NewReader(diff), "")
	if err != nil {
...
+ cut off
+ cut off`
	result, err = gitdiff.ParsePatch(setting.Git.MaxGitDiffLines, setting.Git.MaxGitDiffLineCharacters, setting.Git.MaxGitDiffFiles, strings.NewReader(diff2), "")
	if err != nil {
		t.Errorf("ParsePatch failed: %s", err)
	}
	println(result)
...
+ cut off
+ cut off`
	result, err = gitdiff.ParsePatch(setting.Git.MaxGitDiffLines, setting.Git.MaxGitDiffLineCharacters, setting.Git.MaxGitDiffFiles, strings.NewReader(diff2a), "")
	if err != nil {
		t.Errorf("ParsePatch failed: %s", err)
	}
	println(result)
...
+ cut off
+ cut off`
	result, err = gitdiff.ParsePatch(setting.Git.MaxGitDiffLines, setting.Git.MaxGitDiffLineCharacters, setting.Git.MaxGitDiffFiles, strings.NewReader(diff3), "")
	if err != nil {
		t.Errorf("ParsePatch failed: %s", err)
	}
	println(result)
...
func TestDiffLine_CanComment(t *testing.T) {
	assert.False(t, (&gitdiff.DiffLine{Type: gitdiff.DiffLineSection}).CanComment())
	assert.False(t, (&gitdiff.DiffLine{Type: gitdiff.DiffLineAdd, Comments: []*issues_model.Comment{{Content: "bla"}}}).CanComment())
	assert.True(t, (&gitdiff.DiffLine{Type: gitdiff.DiffLineAdd}).CanComment())
	assert.True(t, (&gitdiff.DiffLine{Type: gitdiff.DiffLineDel}).CanComment())
	assert.True(t, (&gitdiff.DiffLine{Type: gitdiff.DiffLinePlain}).CanComment())
}

func TestDiffLine_GetCommentSide(t *testing.T) {
	assert.Equal(t, "previous", (&gitdiff.DiffLine{Comments: []*issues_model.Comment{{Line: -3}}}).GetCommentSide())
	assert.Equal(t, "proposed", (&gitdiff.DiffLine{Comments: []*issues_model.Comment{{Line: 3}}}).GetCommentSide())
}
...
func TestGetDiffRangeWithWhitespaceBehavior(t *testing.T) {
	...
	for _, behavior := range []string{"-w", "--ignore-space-at-eol", "-b", ""} {
		diffs, err := gitdiff.GetDiff(gitRepo,
			&gitdiff.DiffOptions{
...
func TestDiffToHTML_14231(t *testing.T) {
	...
	output := diffToHTML("main.v", diffRecord, gitdiff.DiffLineAdd)
...
func TestNoCrashes(t *testing.T) {
	...
	for _, testcase := range tests {
		...
		gitdiff.ParsePatch(setting.Git.MaxGitDiffLines, setting.Git.MaxGitDiffLineCharacters, setting.Git.MaxGitDiffFiles, strings.NewReader(testcase.gitdiff), "")
	}
}
