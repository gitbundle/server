...
import (
	...
	webhook_services "github.com/gitbundle/server/pkg/webhook/manager"
)
...
func sendReleaseHook(doer *user_model.User, rel *release_model.Release, action api.HookReleaseAction) {
...
func (m *webhookNotifier) NotifyNewRelease(rel *release_model.Release) {
	sendReleaseHook(rel.Publisher, rel, api.HookReleasePublished)
}

func (m *webhookNotifier) NotifyUpdateRelease(doer *user_model.User, rel *release_model.Release) {
	sendReleaseHook(doer, rel, api.HookReleaseUpdated)
}

func (m *webhookNotifier) NotifyDeleteRelease(doer *user_model.User, rel *release_model.Release) {
