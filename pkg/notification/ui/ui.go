...
func (ns *notificationService) handle(data ...queue.Data) []queue.Data {
	for _, datum := range data {
		...
		if err := repoman.CreateOrUpdateIssueNotifications(opts.IssueID, opts.CommentID, opts.NotificationAuthorID, opts.ReceiverID); err != nil {
...
func (ns *notificationService) NotifyRepoPendingTransfer(doer, newOwner *user_model.User, repo *repo_model.Repository) {
	if err := repoman.CreateRepoTransferNotification(doer, newOwner, repo); err != nil {
