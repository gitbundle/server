...
func (o *OAuth2) userIDFromToken(req *http.Request, store DataStore) int64 {
	...
	t, err := access_token_model.GetAccessTokenBySHA(tokenSHA)
	if err != nil {
		if !access_token_model.IsErrAccessTokenNotExist(err) && !access_token_model.IsErrAccessTokenEmpty(err) {
			log.Error("GetAccessTokenBySHA: %v", err)
		}
		return 0
	}
	t.UpdatedUnix = timeutil.TimeStampNow()
	if err = access_token_model.UpdateAccessToken(t); err != nil {
...
func (o *OAuth2) Verify(req *http.Request, w http.ResponseWriter, store DataStore, sess SessionStore) *user_model.User {
	...
	log.Trace("OAuth2 Authorization: Signin user[%-v]", user.Name)
	return user
}
