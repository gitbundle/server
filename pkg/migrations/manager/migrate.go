...
// MigrateOptions is equal to schemas.MigrateOptions
type MigrateOptions = schemas.MigrateOptions

var (
	factories []schemas.DownloaderFactory
...
func RegisterDownloaderFactory(factory schemas.DownloaderFactory) {
...
func IsMigrateURLAllowed(remoteURL string, doer *user_model.User) error {
	...
	if err != nil {
		return &repoman.ErrInvalidCloneAddr{IsURLError: true, Host: remoteURL}
	}

	if u.Scheme == "file" || u.Scheme == "" {
		if !doer.CanImportLocal() {
			return &repoman.ErrInvalidCloneAddr{
	...
	if u.Scheme == "git" && u.Port() != "" &&
		...
		return &repoman.ErrInvalidCloneAddr{Host: u.Host, IsURLError: true}
	}

	if u.Opaque != "" ||
		u.Scheme != "" && u.Scheme != "http" && u.Scheme != "https" && u.Scheme != "git" {
		return &repoman.ErrInvalidCloneAddr{
			Host:               u.Host,
	...
	if blockList.MatchHostName(hostName) || ipBlocked {
		blockedError = &repoman.ErrInvalidCloneAddr{Host: u.Host, IsPermissionDenied: true}
	}
	// if we have an allow-list, check the allow-list first
	if !allowList.IsEmpty() {
		if !allowList.MatchHostName(hostName) && !ipAllowed {
			return &repoman.ErrInvalidCloneAddr{Host: u.Host, IsPermissionDenied: true}
		}
	}
...
func newDownloader(
	...
	ownerName string,
	opts schemas.MigrateOptions,
) (schemas.Downloader, error) {
	var (
		downloader schemas.Downloader
		err        error
	)
	...
	if setting.Migrations.MaxAttempts > 1 {
		downloader = schemas.NewRetryDownloader(
			ctx,
			downloader,
...
func migrateRepository(
	...
	if messenger == nil {
		messenger = schemas.NilMessenger
	}

	repo, err := downloader.GetRepoInfo()
	if err != nil {
		if !schemas.IsErrNotSupported(err) {
			return err
		}
	...
	if opts.Issues {
		...
		for i := 1; ; i++ {
			...
			if opts.Comments && !supportAllComments {
				allComments := make([]*schemas.Comment, 0, commentBatchSize)
	...
	if opts.PullRequests {
		...
		for i := 1; ; i++ {
			...
			if opts.Comments {
				...
				// migrate reviews
				allReviews := make([]*schemas.Review, 0, reviewBatchSize)
				for _, pr := range prs {
...
func Init() error {
	...
	blockList = hostmatcher.ParseSimpleMatchList(
		"migrations.BLOCKED_DOMAINS",
		setting.Migrations.BlockedDomains,
	)

	allowList = hostmatcher.ParseSimpleMatchList(
		"migrations.ALLOWED_DOMAINS/ALLOW_LOCALNETWORKS",
