...
var _ schemas.Uploader = &RepositoryDumper{}
...
func NewRepositoryDumper(ctx context.Context, baseDir, repoOwner, repoName string, opts schemas.MigrateOptions) (*RepositoryDumper, error) {
...
func (g *RepositoryDumper) CreateRepo(repo *schemas.Repository, opts schemas.MigrateOptions) error {
...
func (g *RepositoryDumper) CreateMilestones(milestones ...*schemas.Milestone) error {
	var err error
	if g.milestoneFile == nil {
...
func (g *RepositoryDumper) CreateLabels(labels ...*schemas.Label) error {
	var err error
	if g.labelFile == nil {
...
func (g *RepositoryDumper) CreateReleases(releases ...*schemas.Release) error {
...
func (g *RepositoryDumper) CreateIssues(issues ...*schemas.Issue) error {
	var err error
	if g.issueFile == nil {
...
func (g *RepositoryDumper) CreateComments(comments ...*schemas.Comment) error {
...
func (g *RepositoryDumper) CreatePullRequests(prs ...*schemas.PullRequest) error {
	for _, pr := range prs {
		// download patch file
		err := func() error {
...
func (g *RepositoryDumper) CreateReviews(reviews ...*schemas.Review) error {
...
func DumpRepository(ctx context.Context, baseDir, ownerName string, opts schemas.MigrateOptions) error {
...
func updateOptionsUnits(opts *schemas.MigrateOptions, units []string) error {
	if len(units) == 0 {
		opts.Wiki = true
		opts.Issues = true
...
func RestoreRepository(ctx context.Context, baseDir, ownerName, repoName string, units []string, validation bool) error {
	...
	migrateOpts := schemas.MigrateOptions{
