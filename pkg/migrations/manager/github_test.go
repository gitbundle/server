...
func TestGitHubDownloadRepo(t *testing.T) {
	migrations.GithubLimitRateRemaining = 3 // Wait at 3 remaining since we could have 3 CI in //
	downloader := migrations.NewGithubDownloaderV3(context.Background(), "https://github.com", "", "", os.Getenv("GITHUB_READ_TOKEN"), "go-gitea", "test_repo")
	err := downloader.RefreshRate()
	assert.NoError(t, err)

	repo, err := downloader.GetRepoInfo()
	assert.NoError(t, err)
	assertRepositoryEqual(t, &schemas.Repository{
		Name:          "test_repo",
		Owner:         "go-gitea",
	...
	assert.NoError(t, err)
	assertMilestonesEqual(t, []*schemas.Milestone{
		{
			Title:       "1.0.0",
	...
	assert.NoError(t, err)
	assertLabelsEqual(t, []*schemas.Label{
		{
			Name:        "bug",
			Color:       "d73a4a",
	...
	assert.NoError(t, err)
	assertReleasesEqual(t, []*schemas.Release{
		{
			TagName:         "v0.9.99",
	...
	assertIssuesEqual(t, []*schemas.Issue{
		{
			Number:     1,
	...
	comments, _, err := downloader.GetComments(&schemas.Issue{Number: 2, ForeignIndex: 2})
	assert.NoError(t, err)
	assertCommentsEqual(t, []*schemas.Comment{
		{
			IssueIndex: 2,
			PosterID:   1669571,
			PosterName: "mrsdizzie",
	...
	assert.NoError(t, err)
	assertPullRequestsEqual(t, []*schemas.PullRequest{
		{
			Number:     3,
	...
	reviews, err := downloader.GetReviews(&schemas.PullRequest{Number: 3, ForeignIndex: 3})
	assert.NoError(t, err)
	assertReviewsEqual(t, []*schemas.Review{
		{
			ID:           315859956,
			IssueIndex:   3,
			ReviewerID:   42128690,
	...
	reviews, err = downloader.GetReviews(&schemas.PullRequest{Number: 4, ForeignIndex: 4})
	assert.NoError(t, err)
	assertReviewsEqual(t, []*schemas.Review{
		{
			ID:           338338740,
			...
			State:        schemas.ReviewStateApproved,
			Comments: []*schemas.ReviewComment{
				{
					ID:        363017488,
		...
		},
		{
			...
			State:        schemas.ReviewStateChangesRequested,
		...
		},
		{
			...
			State:        schemas.ReviewStateCommented,
			Comments: []*schemas.ReviewComment{
				{
					ID:        363029944,
