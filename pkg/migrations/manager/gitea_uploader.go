...
import (
	...
	"github.com/gitbundle/modules/uri"
...
var _ schemas.Uploader = &GiteaLocalUploader{}
...
func (g *GiteaLocalUploader) MaxBatchInsertSize(tp string) int {
	...
	case "release":
		return db.MaxBatchInsertSize(new(release_model.Release))
	case "pullrequest":
...
func (g *GiteaLocalUploader) CreateRepo(repo *schemas.Repository, opts schemas.MigrateOptions) error {
	...
	if opts.MigrateToRepoID <= 0 {
		r, err = repo_module.CreateRepository(g.doer, owner, repoman.CreateRepoOptions{
			Name:           g.repoName,
	...
	r, err = repo_module.MigrateRepositoryGitData(g.ctx, owner, r, schemas.MigrateOptions{
		RepoName:       g.repoName,
		Description:    repo.Description,
		OriginalURL:    repo.OriginalURL,
		GitServiceType: opts.GitServiceType,
...
func (g *GiteaLocalUploader) CreateMilestones(milestones ...*schemas.Milestone) error {
	...
	err := migration_model.InsertMilestones(mss...)
	if err != nil {
		return err
	}
...
func (g *GiteaLocalUploader) CreateLabels(labels ...*schemas.Label) error {
...
func (g *GiteaLocalUploader) CreateReleases(releases ...*schemas.Release) error {
	rels := make([]*release_model.Release, 0, len(releases))
	...
	return migration_model.InsertReleases(rels...)
}
...
func (g *GiteaLocalUploader) CreateIssues(issues ...*schemas.Issue) error {
	...
	if len(iss) > 0 {
		if err := migration_model.InsertIssues(iss...); err != nil {
			return err
		}
...
func (g *GiteaLocalUploader) CreateComments(comments ...*schemas.Comment) error {
	...
	return migration_model.InsertIssueComments(cms)
}

// CreatePullRequests creates pull requests
func (g *GiteaLocalUploader) CreatePullRequests(prs ...*schemas.PullRequest) error {
	...
	if err := migration_model.InsertPullRequests(gprs...); err != nil {
		return err
	}
	for _, pr := range gprs {
...
func (g *GiteaLocalUploader) newPullRequest(pr *schemas.PullRequest) (*issues_model.PullRequest, error) {
	var labels []*issues_model.Label
...
func convertReviewState(state string) issues_model.ReviewType {
	switch state {
	case schemas.ReviewStatePending:
		return issues_model.ReviewTypePending
	case schemas.ReviewStateApproved:
		return issues_model.ReviewTypeApprove
	case schemas.ReviewStateChangesRequested:
		return issues_model.ReviewTypeReject
	case schemas.ReviewStateCommented:
		return issues_model.ReviewTypeComment
	case schemas.ReviewStateRequestReview:
...
func (g *GiteaLocalUploader) CreateReviews(reviews ...*schemas.Review) error {
	...
	for _, review := range reviews {
		...
		for _, comment := range review.Comments {
			...
			go func(comment *schemas.ReviewComment) {
...
func (g *GiteaLocalUploader) Rollback() error {
	if g.repo != nil && g.repo.ID > 0 {
		g.gitRepo.Close()
		if err := repoman.DeleteRepository(g.doer, g.repo.OwnerID, g.repo.ID); err != nil {
			return err
		}
	}
	return nil
...
func (g *GiteaLocalUploader) Finish() error {
	...
	if err := repoman.UpdateRepoStats(g.ctx, g.repo.ID); err != nil {
		return err
	}
