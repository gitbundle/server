...
var (
	_ schemas.Downloader        = &GiteaDownloader{}
	_ schemas.DownloaderFactory = &GiteaDownloaderFactory{}
)
...
func (f *GiteaDownloaderFactory) New(ctx context.Context, opts schemas.MigrateOptions) (schemas.Downloader, error) {
...
func NewGiteaDownloader(ctx context.Context, baseURL, repoPath, username, password, token string) (*GiteaDownloader, error) {
	...
	if err = giteaClient.CheckServerVersionConstraint(""); err != nil {
...
func (g *GiteaDownloader) GetRepoInfo() (*schemas.Repository, error) {
	if g == nil {
...
func (g *GiteaDownloader) GetMilestones() ([]*schemas.Milestone, error) {
	milestones := make([]*schemas.Milestone, 0, g.maxPerPage)
...
func (g *GiteaDownloader) convertGiteaLabel(label *gitea_sdk.Label) *schemas.Label {
...
func (g *GiteaDownloader) GetLabels() ([]*schemas.Label, error) {
	labels := make([]*schemas.Label, 0, g.maxPerPage)
...
func (g *GiteaDownloader) convertGiteaRelease(rel *gitea_sdk.Release) *schemas.Release {
	r := &schemas.Release{
		TagName:         rel.TagName,
...
func (g *GiteaDownloader) GetReleases() ([]*schemas.Release, error) {
	releases := make([]*schemas.Release, 0, g.maxPerPage)
...
func (g *GiteaDownloader) getIssueReactions(index int64) ([]*schemas.Reaction, error) {
...
func (g *GiteaDownloader) getCommentReactions(commentID int64) ([]*schemas.Reaction, error) {
	var reactions []*schemas.Reaction
	if err := g.client.CheckServerVersionConstraint(">=1.11"); err != nil {
...
func (g *GiteaDownloader) GetIssues(page, perPage int) ([]*schemas.Issue, bool, error) {
...
func (g *GiteaDownloader) GetComments(commentable schemas.Commentable) ([]*schemas.Comment, bool, error) {
...
func (g *GiteaDownloader) GetPullRequests(page, perPage int) ([]*schemas.PullRequest, bool, error) {
	if perPage > g.maxPerPage {
		perPage = g.maxPerPage
	}
...
func (g *GiteaDownloader) GetReviews(reviewable schemas.Reviewable) ([]*schemas.Review, error) {
	...
	for i := 1; ; i++ {
		...
		for _, pr := range prl {
			...
			var reviewComments []*schemas.ReviewComment
			for i := range rcl {
				...
				reviewComments = append(reviewComments, &schemas.ReviewComment{
					ID:        rcl[i].ID,
					Content:   rcl[i].Body,
					TreePath:  rcl[i].Path,
