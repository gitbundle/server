...
func TestGitlabDownloadRepo(t *testing.T) {
	...
	downloader, err := migrations.NewGitlabDownloader(context.Background(), "https://gitlab.com", "gitea/test_repo", "", "", gitlabPersonalAccessToken)
	if err != nil {
	...
	comments, _, err := downloader.GetComments(&schemas.Issue{
		Number:       2,
		ForeignIndex: 2,
	...
	rvs, err := downloader.GetReviews(&schemas.PullRequest{Number: 1, ForeignIndex: 1})
	assert.NoError(t, err)
	...
	rvs, err = downloader.GetReviews(&schemas.PullRequest{Number: 2, ForeignIndex: 2})
	assert.NoError(t, err)
	assertReviewsEqual(t, []*schemas.Review{
		{
			IssueIndex:   2,
...
func convertTestCase(t reviewTestCase) (func(w http.ResponseWriter, r *http.Request), schemas.Review) {
...
func TestGitlabGetReviews(t *testing.T) {
	...
	for _, testCase := range []reviewTestCase{
		...
		rvs, err := downloader.GetReviews(&schemas.Issue{Number: id, ForeignIndex: id})
		assert.NoError(t, err)
		assertReviewsEqual(t, []*schemas.Review{&review}, rvs)
	}
}
