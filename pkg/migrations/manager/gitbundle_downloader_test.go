...
func TestGiteaDownloadRepo(t *testing.T) {
	...
	giteaToken := os.Getenv("GITBUNDLE_TOKEN")
	if giteaToken == "" {
		t.Skip("skipped test because GITBUNDLE_TOKEN was not in the environment")
	}
	...
	downloader, err := migrations.NewGiteaDownloader(context.Background(), "https://gitea.com", "gitea/test_repo", "", "", giteaToken)
	if downloader == nil {
	...
	comments, _, err := downloader.GetComments(&schemas.Issue{Number: 4, ForeignIndex: 4})
	assert.NoError(t, err)
	...
	assert.False(t, isEnd)
	assert.Len(t, prs, 3)
	assertPullRequestEqual(t, &schemas.PullRequest{
		Number:      12,
		PosterID:    689,
		PosterName:  "6543",
		PosterEmail: "6543@obermui.de",
	...
	reviews, err := downloader.GetReviews(&schemas.Issue{Number: 7, ForeignIndex: 7})
	assert.NoError(t, err)
