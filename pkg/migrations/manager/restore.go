...
func (r *RepositoryRestorer) GetRepoInfo() (*schemas.Repository, error) {
...
func (r *RepositoryRestorer) GetMilestones() ([]*schemas.Milestone, error) {
	milestones := make([]*schemas.Milestone, 0, 10)
	p := filepath.Join(r.baseDir, "milestone.yml")
	err := schemas.Load(p, &milestones, r.validation)
	if err != nil {
		if os.IsNotExist(err) {
			return nil, nil
		}
...
func (r *RepositoryRestorer) GetReleases() ([]*schemas.Release, error) {
	releases := make([]*schemas.Release, 0, 10)
...
func (r *RepositoryRestorer) GetLabels() ([]*schemas.Label, error) {
	labels := make([]*schemas.Label, 0, 10)
...
func (r *RepositoryRestorer) GetIssues(page, perPage int) ([]*schemas.Issue, bool, error) {
	issues := make([]*schemas.Issue, 0, 10)
	p := filepath.Join(r.baseDir, "issue.yml")
	err := schemas.Load(p, &issues, r.validation)
	if err != nil {
		if os.IsNotExist(err) {
			return nil, true, nil
		}
...
func (r *RepositoryRestorer) GetComments(commentable schemas.Commentable) ([]*schemas.Comment, bool, error) {
	comments := make([]*schemas.Comment, 0, 10)
...
func (r *RepositoryRestorer) GetPullRequests(page, perPage int) ([]*schemas.PullRequest, bool, error) {
	pulls := make([]*schemas.PullRequest, 0, 10)
...
func (r *RepositoryRestorer) GetReviews(reviewable schemas.Reviewable) ([]*schemas.Review, error) {
	reviews := make([]*schemas.Review, 0, 10)
