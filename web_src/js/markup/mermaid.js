import { isDarkTheme } from '../utils/index.js'
...
export async function renderMermaid() {
  ...
  for (const el of els) {
    ...
    if (
      mermaidMaxSourceCharacters >= 0 &&
      source.length > mermaidMaxSourceCharacters
    ) {
      displayError(
        el,
        new Error(
          `Mermaid source of ${source.length} characters exceeds the maximum allowed length of ${mermaidMaxSourceCharacters}.`
        )
      )
      continue
    }
    ...
    try {
      ...
      mermaid.mermaidAPI.render('mermaid', source, (svgStr) => {
        ...
        if (!heightStr)
          return displayError(el, new Error('Could not determine chart height'))
