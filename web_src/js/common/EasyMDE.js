...
export async function importEasyMDE() {
  ...
  const { default: EasyMDE } = await import(
    /* webpackChunkName: "easymde" */ 'easymde'
  )
  return EasyMDE
}
...
export async function createCommentEasyMDE(textarea, easyMDEOptions = {}) {
  ...
  const easyMDE = new EasyMDE({
    autoDownloadFontAwesome: false, //TODO: should fix font-awesome for postcss-loader
    element: textarea,
    forceSync: true,
    renderingConfig: {
...
export function validateTextareaNonEmpty($textarea) {
  const $mdeInputField = $(
    getAttachedEasyMDE($textarea).codemirror.getInputField()
  )
