<script>
  ...
  import { tick, createEventDispatcher, afterUpdate, beforeUpdate } from 'svelte'
  ...
  let results = []
  export let handleResults = (results) => results
  ...
  $: if (((isFocused && showAllResultsOnFocus) || showAllResultsOnOpen) && value.length === 0) {
    results = data
  ...
  beforeUpdate(() => {
    // console.log($$slots, !hideDropdown, value.length, results.length)
  })
...
<div data-svelte-typeahead bind:this={comboboxRef} role="combobox" aria-haspopup="listbox" aria-controls="{id}-listbox" aria-expanded={showResults} id="{id}-typeahead">
  <Search
    {id}
  ...
  />
  <ul class:svelte-typeahead-list={true} aria-labelledby="{id}-label" id="{id}-listbox">
    {#if showResults}
      {#each results as result, index}
        <!-- {console.log(result)} -->
        <!-- aria-selected={selectedIndex === index || defaultSelectedValue === result.string} -->
        <!-- svelte-ignore a11y-click-events-have-key-events -->
        <!-- svelte-ignore a11y-mouse-events-have-key-events -->
        <li
          id="{id}-result-{index}"
          class:selected={selectedIndex === index || defaultSelectedValue === result.string}
