...
async function showLineButton() {
  if ($('.code-line-menu').length === 0) return
  $('.code-line-button').remove()
  $('.code-view td.lines-code.active')
    .closest('tr')
    .find('td:eq(0)')
    .first()
    .prepend(
      $(
        `<button class="code-line-button btn-outline btn-xs btn">${await svg(
...
export function initRepoCodeView() {
  if ($('.code-view .lines-num').length > 0) {
    $(document)
      .off('click', '.lines-num span')
      .on('click', '.lines-num span', function (e) {
        const $select = $(this)
        let $list
        if ($('div.blame').length) {
