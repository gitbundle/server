...
const viewedStyleClass = 'bg-info'
const viewedCheckboxStyleClass = 'checkbox-info'
...
function refreshViewedFilesSummary() {
  ...
  if (summaryLabel)
    summaryLabel.innerHTML = summaryLabel
      .getAttribute('data-text-changed-template')
...
export function countAndUpdateViewedFiles() {
  ...
  prReview.numberOfViewedFiles = document.querySelectorAll(
    `${viewedCheckboxSelector} > input[type=checkbox][checked]`
  ).length
...
export function initViewedCheckboxListenerFor() {
  for (const form of document.querySelectorAll(
    `${viewedCheckboxSelector}:not([data-has-viewed-checkbox-listener="true"])`
  )) {
    ...
    checkbox.addEventListener('change', function () {
      ...
      if (this.checked) {
        ...
        this.classList.add(viewedCheckboxStyleClass)
        prReview.numberOfViewedFiles++
      } else {
        form.classList.remove(viewedStyleClass)
        this.classList.remove(viewedCheckboxStyleClass)
        prReview.numberOfViewedFiles--
      }
      ...
      setFileFolding(
        parentBox.closest('.file-content'),
        parentBox.querySelector('.fold-file'),
        this.checked
      )
    })
  }
