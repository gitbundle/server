...
import { emojiHTML, emojiKeys, emojiString } from '../common/emoji.js'
  ...
  if (mentions) {
    collections.push({
      ...
      menuItemTemplate: (item) => {
        return `<div class="tribute-item">${emojiHTML(
          item.original
        )}<span>${htmlEscape(item.original)}</span></div>`
      }
    })
  }
          ...
          <div class="tribute-item">
            ...
            ${
              item.original.fullname && item.original.fullname !== ''
                ? `<span class="fullname">${htmlEscape(
...
export default async function attachTribute(
  elementOrNodeList,
  { mentions, emoji } = {}
) {
  if (!window.config.requireTribute || !elementOrNodeList) return
  const nodes = Array.from(
    'length' in elementOrNodeList ? elementOrNodeList : [elementOrNodeList]
  )
  ...
  const emojiNodes = nodes.filter((node) => {
    return (
      emoji || node.id === 'content' || node.classList.contains('emoji-input')
    )
  })
  const uniqueNodes = uniq([...mentionNodes, ...emojiNodes])
  if (!uniqueNodes.length) return

  const { default: Tribute } = await import(
    /* webpackChunkName: "tribute" */ 'tributejs'
  )
