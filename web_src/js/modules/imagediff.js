...
export default function initImageDiff() {
  ...
  $('.image-diff').each(function () {
    ...
    const diffContainerWidth = Math.max(
      $container.closest('.diff-file-box').width() - 300,
      100
    )
    ...
    function initSideBySide(sizes) {
      ...
      const widthChanged =
        sizes.image1.length !== 0 &&
        sizes.image2.length !== 0 &&
        sizes.image1[0].naturalWidth !== sizes.image2[0].naturalWidth
      const heightChanged =
        sizes.image1.length !== 0 &&
        sizes.image2.length !== 0 &&
        sizes.image1[0].naturalHeight !== sizes.image2[0].naturalHeight
      if (sizes.image1.length !== 0) {
        $container
          .find('.bounds-info-after .bounds-info-width')
          .text(`${sizes.image1[0].naturalWidth}px`)
          .addClass(widthChanged ? 'text-green-800' : '')
        $container
          .find('.bounds-info-after .bounds-info-height')
          .text(`${sizes.image1[0].naturalHeight}px`)
          .addClass(heightChanged ? 'text-green-800' : '')
      }
      if (sizes.image2.length !== 0) {
        $container
          .find('.bounds-info-before .bounds-info-width')
          .text(`${sizes.image2[0].naturalWidth}px`)
          .addClass(widthChanged ? 'text-red-800' : '')
        $container
          .find('.bounds-info-before .bounds-info-height')
          .text(`${sizes.image2[0].naturalHeight}px`)
          .addClass(heightChanged ? 'text-red-800' : '')
      }
      ...
      sizes.image1.parent().css({
        // margin: `${sizes.ratio[1] * factor + 15}px ${
        //   sizes.ratio[0] * factor
        // }px ${sizes.ratio[1] * factor}px`,
      ...
      $container.find('.swipe-bar').on('mousedown', function (e) {
        ...
        $(document).on('mousemove.diff-swipe', (e2) => {
          ...
          const value = Math.max(
            0,
            Math.min(e2.clientX - $swipeFrame.offset().left, width)
          )
