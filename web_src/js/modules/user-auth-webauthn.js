...
export function initUserAuthWebAuthn() {
  if ($('[jq-user-auth-webauthn-prompt]').length === 0) {
    return
  }
  ...
  $.getJSON(`${appSubUrl}/user/webauthn/assertion`, {})
    .done((makeAssertionOptions) => {
      makeAssertionOptions.publicKey.challenge = decode(
      ...
      for (
        ...
        makeAssertionOptions.publicKey.allowCredentials[i].id = decode(
      ...
      navigator.credentials
        ...
        .catch((err) => {
          ...
          if (
            makeAssertionOptions.publicKey.extensions &&
            makeAssertionOptions.publicKey.extensions.appid
          ) {
...
function webAuthnError(errorType, message) {
  ...
  if (errorType === 'general') {
    ...
    const $errorTyped = $(
      `#webauthn-error [data-webauthn-error-msg=${errorType}]`
    )
  ...
  $('#webauthn-error-modal').prop('checked', true)
}
...
function webAuthnRegisterRequest() {
  ...
  $.post(`${appSubUrl}/user/settings/security/webauthn/request_register`, {
    ...
    .done((makeCredentialOptions) => {
      ...
      makeCredentialOptions.publicKey.challenge = decode(
        makeCredentialOptions.publicKey.challenge
      )
      makeCredentialOptions.publicKey.user.id = decode(
      ...
      if (makeCredentialOptions.publicKey.excludeCredentials) {
        for (
          ...
          makeCredentialOptions.publicKey.excludeCredentials[i].id = decode(
