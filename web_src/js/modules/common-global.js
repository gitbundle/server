...
import { attachDropdownAria } from '../features/aria.js'
import $ from '../features/jquery.js'
import { showGlobalErrorMessage } from '../utils/bootstrap.js'
import { mqBinarySearch } from '../utils/index.js'
...
export function initGlobalFormDirtyLeaveConfirm() {
  ...
  if ($('.user.signin').length === 0) {
    $('form:not(.ignore-dirty, [data-svelte-search])').areYouSure()
  }
}
...
export function initGlobalEnterQuickSubmit() {
  $(document)
    .off('keydown', '.js-quick-submit')
    .on('keydown', '.js-quick-submit', (e) => {
...
export function initGlobalButtons() {
  $('[jq-show-panel]').on('click', function () {
    $($(this).data('panel')).removeClass('hidden')
  })

  $('[jq-hide-panel]').on('click', function (event) {
    ...
    if (sel) {
      $(this).closest(sel).addClass('hidden')
      return
    }
