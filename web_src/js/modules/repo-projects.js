...
function updateIssueCount(cards) {
  ...
  const cnt = parent.querySelectorAll('[jq-board-card]').length
  parent.querySelectorAll('[jq-board-card-cnt]')[0].innerText = cnt
}

function moveIssue({ item, from, to, oldIndex }) {
  const columnCards = to.querySelectorAll('[jq-board-card]')
...
async function initRepoProjectSortable() {
  ...
  let boardColumns = mainBoard.querySelectorAll('[jq-board-column]')
  new Sortable(mainBoard, {
    group: 'board-column',
    draggable: '[jq-board-column]',
    filter: '[data-id="0"]',
    animation: 150,
    ghostClass: 'cu-card-ghost',
    onSort: () => {
      boardColumns = mainBoard.querySelectorAll('[jq-board-column]')
  ...
  for (const boardColumn of boardColumns) {
    const boardCardList = boardColumn.querySelectorAll('[jq-board]')[0]
...
export function initRepoProject() {
  if (!$('[jq-repository-view-project]').length) {
    return
  }

  initRepoProjectSortable()

  $('[jq-edit-project-board]').each(function () {
    const projectHeader = $(this).closest('[jq-board-column-header]')
    const projectTitleLabel = projectHeader.find('[jq-board-label]')
    const projectTitleInput = $(this).find('input[name="title"]')
    const projectColorInput = $(this).find('input[name="color"]')
    const boardColumn = $(this).closest('[jq-board-column]')
