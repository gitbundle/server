...
// We also introduce a `gitbundle-fmt` command, it does better import formatting than gofmt/goimports. `gitbundle-fmt` calls `gofmt` internally.
...
func newFileCollector(fileFilter string, batchSize int) (*fileCollector, error) {
	...
	if fileFilter == "go-own" {
		...
		co.excludePatterns = append(co.excludePatterns, regexp.MustCompile(`integrations/gitbundle-repositories-meta`))
	...
	for _, files := range fileBatches {
		...
		case "gitbundle-fmt":
			if containsString(subArgs, "-d") {
				log.Print("the -d option is not supported by gitbundle-fmt")
			}
